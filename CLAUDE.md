...
`paths.py` (repo-relative paths, the THREE climatology resolvers -- the baseline, the bootstrap, and the best available of the two with the stage it chose -- each of them PER RUNG, because the declaration takes a rung-to-path mapping beside the scalar form and a climatology at another rung is another world's climate rather than an earlier stage of this one's, and its clean-I/O and configured-grid guards), `gridding.py` (mesh-to-grid, the one grid
