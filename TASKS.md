...
| BIO-20 | Add a no-simulation regression suite for the Vesper adapter and consumers: binary round-trip, coordinate bijection, year cycling, precipitation conservation, incomplete-output rejection, equilibrium reduction, and rootable/lake/two-band compositing | `biosphere/notes/modelling-gap-audit.md` finding 10 | open; keep the interface testable without building or launching LPJ-GUESS, adding fixtures alongside BIO-11 through BIO-19 as their contracts land [step: lpj_driver, lpj_run, surface_albedo, surface_roughness] |
