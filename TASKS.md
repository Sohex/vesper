...
| CLIM-42 | Add a CH4 and N2O band to the longwave. `config/planet.yaml` says the shape of it -- "adding either means adding a band, not a key" -- and CLIM-41 measured that the term outranks every other item in the budget | `exoplasim/notes/trace-gas-absorbers.md` section 5, `analysis/error_budget.json` | open. It lands on `radmod.f90`, the same file CLIM-39 is changing, so sequence the two rather than racing them. Sasamori (1968) is a broadband parameterisation and CH4's 7.7 um band sits inside the window it already treats. Justified because the gas is there, NOT because it improves an agreement: the bound says the term is large, not that any value is right, and a band added to close a comparison is failure-modes class 16 [step: rebuild_binaries]
| CLIM-43 | Settle whether TROPOSPHERIC N2O is enhanced at this star, which is the whole width of the CH4/N2O budget item: read Rugheimer et al. (2013) Fig. 7's per-grid-star surface mixing ratios at its 5000 K point, which is this host to within 35 K | `exoplasim/notes/trace-gas-absorbers.md` section 3 | open, and cheap -- the paper is on disk and read. Segura's Fig. 12 reads N2O higher for a K2V at 1 PAL; Rugheimer says "up to about 20 km there is no significant difference between stellar types" because O3 shields N2O from photolysis below that, which would keep the enhancement out of the forcing. A mechanism against a value read off a log axis, so case A is favoured, but the two have not been reconciled. Rugheimer also closes both caveats the estimate carries: 250 K grid spacing instead of interpolating from 4620 K, IUE-OBSERVED ultraviolet instead of a quiet modelled spectrum, and epsilon Eridani itself as a grid star [step: design_flux]
...
| CLIM-19 | `hdiffk` = 300 gives 3.86x more high-latitude warming per unit diffusivity than the line through 1000 and 3000, and slightly more warming outright than 1000 despite 3.3x less diffusion. Both points are resolved 15 of 15 orbits, so it is not noise, and the declared linearity falsifier fired on it | `exoplasim/notes/forcing-bundle-predictions.md`, the CLIM-16 bracket | open. The operator is linear in the field and in `hdiffk` by construction and the two upper points agree with that to 0.3%, so the departure is in the RESPONSE and not the term. First place to look is the ice state: 300 and 3000 both lose sea ice while 1000 loses none, so an ice-albedo feedback that has not equilibrated in fifteen orbits would produce exactly this non-monotonicity. Distinguish by running the three arms far enough for the ice edge to settle, or by repeating 300 and 1000 from a different restart: internal variability in the ice state repeats differently, a real response repeats the same [step: baseline_run] |
...
| GW-7 | Pair the groundwater solver's faces and areas to ONE dual: derive face widths from the centroidal dual Orogen already uses, rather than reconstructing Voronoi faces and pairing them with `cell_area`. Separately, correct the fork comment that calls triangle centroids "Voronoi vertices on the sphere", which is what made the choice invisible | `notes/audits/mesh-dual-area.md` | open. NOT an Orogen area change, and an attempt at one was reverted: both duals tile the sphere exactly, `cell_area` is the area Orogen's own basin catalogue, hypsometry and `basins.drainageConsistency` identity are computed in, and a naive circumcentre routine LOSES accuracy as the mesh refines -- 1e-8 at 20k regions against 2.9e-5 at 2.5M, because slivers tangle the polygon and `Math.abs` folds add positively. Per cell the two duals differ by more than 10% on 69% of regions; per basin by 1.5%; every current consumer aggregates, so nothing built is wrong [step: hydrography] |
| GW-11 | `sum(cell_area)` on `precarve-craton` is 0.068% above `4 pi R^2` at the manifest's `planet.radiusKm` of 7645.2 km, implying 7647.8. A freshly built sphere at the same region count sums exactly, so the dual does not explain it | `notes/audits/mesh-dual-area.md` section 3 | open, small, and unexplained. It matters because `cell_area` is the area Orogen's basin areas are denominated in, so a radius discrepancy propagates to every catchment figure at the same 0.068%. Check what `planet.radiusKm` was at generation against what the manifest records [step: orogen] |
...
| GW-10 | The divide test compares a groundwater trace on the raw surface against `regions.nc`'s `terminal`, which is a priority flood on the FILLED surface, so no solver can pass it and GW-1 has no working catchment check | `hydrography/notes/mesh-geometry.md` | open, and it is the check GW-1 most needs: the reduction identity and closure both passed while the operator was 178x wrong. A correct version compares against a surface-water catchment derived the same way the groundwater trace is [step: hydrography] |
