...
| DUST-3 | Decide what to do now the reopening test has fired by 7.4x. `notes/dust.md` pre-committed to 'the emission scheme belongs in the model' above a land-mean optical depth of 0.10, and the measured central value is 0.740. That is a larger architectural change than prescribing a field, and the same note rules out the Generic PCM at two orders of magnitude in runtime plus a pipeline rewrite | `notes/dust.md` | open, NEEDS A DECISION. The three options are a prescribed field anyway with the inconsistency declared, an emission scheme written into the ExoPlaSim fork, or accepting the Generic PCM's cost. Not a call to make inside a component |
