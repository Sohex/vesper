...
| CLIM-16 | Ocean horizontal heat diffusion has a namelist switch, `nhdiff` with `hdiffk`, and the error budget records no-q-flux as STRUCTURAL with "no cheap version exists" | `notes/audits/absent-and-inherited-physics.md` finding 1, `analysis/error_budget.json` other_items | open, and PARTLY MEASURED 2026-08-20. One arm ran in the A3 bundle at `nhdiff = 1`, `hdiffk = 1000`: the global flux term is -0.024 W/m2, confirming the predicted 0.00 W/m2 by construction, so enabling ocean horizontal diffusion moves no energy globally and cannot be argued for or against on that. WHAT IS NOT DONE IS THE ROW'S ACTUAL ASK. `hdiffk` was run at ONE value, not bracketed, so there is no spread to report; and the term the diffusivity actually moves, the +0.03 to +0.12 K via the ice edge, came out at -0.016 K against a two-orbit resolution floor of about 0.5 W/m2, which is neither confirmation nor refutation. Both gaps have the same cause and the same fix: arms across a declared `hdiffk` bracket, run long enough that an ice-edge term of a tenth of a kelvin sits above the floor. The plumbing is done -- `ocean.horizontal_diffusion` and `ocean.horizontal_diffusivity_m2_s` reach `oceanmod_nl` through `configure_otherargs`, so this is a segment length and a bracket, not code [step: baseline_run] |
