...
| GW-14 | Decide whether "no net surface runoff therefore never carves" is the intended semantics of `carve_verdict.py`. Runoff is `max(P - E, 0)` per cell, so a catchment where evaporative demand meets precipitation everywhere delivers exactly zero, the aridity index `(E_lake - P_lake) / runoff` is infinite, and the basin fails `index <= critical_aridity_index` whatever its geometry. **605 basins are in that state** | `hydrography/notes/water-table-convergence.md`, archive GW-4 | open, and it is what the whole groundwater carve result turns on. Defensible as written -- a basin with no surface water never overflows and never incises its outlet -- so this is a decision to confirm rather than a defect to fix, and the row exists because 605 is a large share of the catalogue and nothing states the choice. What it must NOT be answered with is the flip count, which is downstream of it [step: carve_verdict] |
