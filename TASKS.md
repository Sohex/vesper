...
| OCN-2 | Verify the prescribed ocean heat transport channel end to end with a field whose answer is known, BEFORE anything computes a real one. `nfluko = 1` makes `oceanmod.f90` read surface code 903 as a monthly W/m2 field and `addfc` apply it under the header comment "prescribed advection"; `close_ocean_energy.py` already reads the same code out of the `ocean_output` stream and records it as identically zero today. A zonally uniform dipole integrating to zero over the ocean is a test with a right answer: what the ocean reports accumulating must equal what was written, and the global integral must stay zero | `notes/audits/ocean-and-marine-biosphere.md` finding 2 | open, and instrumentation rather than physics, so it is a diagnostic segment and not an orbit. Cheapest row here and it is what makes OCN-3 legible: without it, an offline model's first field would arrive with no way to separate a wrong field from a wrong channel. It also converts the error budget's no-q-flux item from STRUCTURAL with "no cheap version exists" into something with a route, which `nhdiff` bounded but did not supply [step: bootstrap_run] |
| OCN-3 | Scope MITgcm as the offline circulation host: whether it builds on this host, what its sphere radius, gravity and rotation parameters take, what a coarse global configuration on this world's bathymetry out of `source/` would look like, and what a spin-up costs in wall clock. EXPLORATION: the deliverable is a viability verdict and a cost, not a run | `notes/audits/ocean-and-marine-biosphere.md` section 6 | open. Offline is the only affordable coupling class, because equilibrating a deep ocean takes thousands of model years against orbits priced in wall-clock hours; the pattern to copy is aeolian, downstream of one climate and upstream of the next. Chosen over the NEMO family because adopting PISCES, MEDUSA or PlankTOM means adopting NEMO, whose configuration machinery is harder to move off Earth. Nothing has been built or measured yet, which is what this row is for |
| OCN-4 | Scope Darwin on that host: tracer count against cost, what the trait space has to be DECLARED as for a world that is not Earth, what Earth-specific content survives inside it, and its licence. EXPLORATION: the deliverable is a scoping note | `notes/audits/ocean-and-marine-biosphere.md` section 6 | open. Chosen over MARBL for a recorded reason rather than an engineering one: MARBL is the better-engineered library, being driver-agnostic by design, and its ecology is a small set of fixed Earth plant functional types, two of which are specific Earth evolutionary inventions. That is the problem `biosphere/README.md` already records for LPJ-GUESS's shipped PFTs. Trait-based is what makes the modelled community composition an output. Iteration is cheap because the marine biosphere is a LEAF, so a trait-space pass costs no climate run |
| OCN-5 | DECLARE the forcing procedure and the exit predicate for the ocean transport loop before its first pass, and add the loop and its steps to `config/pipeline.yaml` in the same commit. An ocean model forced by a climatology a zero-transport slab produced, and restored toward that climatology's own sea surface temperature, returns a transport of zero by construction, so it would reproduce the answer it was meant to supply | `notes/audits/ocean-and-marine-biosphere.md` section 7 | open, and a DECISION rather than a measurement, which is why it is a task and the run is not. Sequencing is the other half of it and is the part that bites: a transport field moves the global mean, which moves the design flux, which moves the carve verdict, so the ocean PHYSICS is inside loop A and lands before a carve or invalidates one. The ocean BIOLOGY is outside loop A and carries no such constraint |
| OCN-6 | Establish what shortwave penetration into the modelled ocean would cost as code, and what it is worth once there are layers to put it in. `mksst` adds the whole atmospheric heat flux to level 1 and lower levels are reached only by `vdiffkl`, so nothing resolves where in the water column the shortwave is absorbed. EXPLORATION: the deliverable is a cost and a bound | `notes/audits/ocean-and-marine-biosphere.md` finding 5b | open, and ORDERED after OCN-1 rather than merged into it: layers without a penetration profile is a coherent configuration and a profile without layers is not. This is the pathway by which a modelled ocean ecosystem would reach the simulated climate most strongly, and it is the one pathway in the note that could not be priced, because pricing it requires writing it. Recording that it is unpriced is the point [step: rebuild_binaries] |
| OCN-7 | Cost a spatially varying two-band ocean albedo. `seamod.f90:155-158` applies `doceanalb(1)` and `doceanalb(2)` as namelist scalars everywhere `dls < 0.5`, so there is no field and no surface code for one; the machinery to copy is `build_surface_dust.py` and the 1811 code set. EXPLORATION: the deliverable is what the surfcode, the writer and the config key cost | `notes/audits/ocean-and-marine-biosphere.md` finding 5a | open, and the expected outcome is that it does NOT earn its place alone. Priced at 0.014 to 0.28 K, below the smallest item resident in the error budget, and discounted a further 0.712 by this host putting 0.3824 of its flux below 0.75 um against a 5772 K Planck's 0.5374, which is where a blue-and-red pigment's leverage lives. The row exists so the costing is on the record and so the pathway can be taken cheaply if a spatial ocean albedo is ever wanted for another reason [step: surface_albedo] |
| OCN-8 | Cost an aerosol-cloud pathway. `radmod.f90` carries no droplet number, no effective radius and no aerosol-cloud term of any kind, and `aeromod.f90` is the FFSL transport scheme only, so a biogenic sulfur route to cloud albedo has nothing to hook onto. EXPLORATION: the deliverable is what the module costs and what the radiation scheme would have to grow to carry it | `notes/audits/ocean-and-marine-biosphere.md` finding 5c | open, and the expected outcome is out of scope for now. Recorded rather than dropped for two reasons. It is a MODULE and not a field, which is a different class of cost from CLIM-40's prescribed species and should not be mistaken for one. And the same absence bounds what any biogenic aerosol in this project can ever be worth, dust and sea salt included, so the answer is reusable well beyond the ocean [step: rebuild_binaries] |
| OCN-9 | Fix the pre-carve reading in `config/planet.yaml`'s `ocean` block, and sweep for the same number read in the same tense elsewhere. The block argues its salinity direction from the exorheic share being under half Earth's per unit ocean area, which holds only with zero carving: all 3,621 basins have an exit and every spill chain reaches the sea, so the all-carve limit is 1.0000 of land, and the middle of the sweep sits within ten percent of Earth | `notes/audits/ocean-and-marine-biosphere.md` finding 4 | open. Nothing numeric moves, the block being DECLARED at Earth's 34.7 psu, but that sentence is the justification for bracketing the freezing point and the direction it derives, fresher ocean and therefore more simulated sea ice, does not survive the middle of the range. CLAUDE.md rule 9 names this class and the sweep is the point of the row rather than the one instance: `brine_paths.py`, the evaporite classes and the phosphorus routing are all built on the same pre-carve terrain and describe the same floor |
