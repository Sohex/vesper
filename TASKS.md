...
| HYD-5 | Put the sill's own rock into the retain mapping: `basins.nc` carries `spill_region` and the export carries erodibility per region, so the K in `K Q^m S^n` is available and unused. Stock and Montgomery measure K across five orders of magnitude by lithology, which is wider than the discharge spread it currently rests on | `hydrography/notes/retain-fraction.md` | open -- needs no climate, but there is no verdict to check it against until the re-baseline |
