...
| GW-3 | Calibrate against Earth: the same code on Earth topography, Earth recharge and GLHYMPS permeability, scored against Fan et al. (2013)'s well compilation, with the threshold declared before the data is scored | `hydrography/notes/groundwater-scoping.md` section 9, `references/INDEX.md` | open, blocked on ONE input. Obtainable: Copernicus DEM 90 m from the AWS registry, the HYD-7 route; GLHYMPS at Borealis `10.5683/SP2/DLGXYO` under CC-BY; a recharge candidate in Berghuijs et al. (2022) on Zenodo, unread on whether its aridity-class exclusions drop the arid regions that are the only interesting ones here. Fan's SIMULATED field is also live and downloadable at `thredds-gfnl.usc.es/thredds/catalog/GLOBALWTDFTP` -- Miguez-Macho's own institution -- as continental annual and monthly means. **The WELL OBSERVATIONS are what is missing**: `glowasis.deltares.nl` is NXDOMAIN, the Science supplement 403s, and the exact Database S1 filenames now in `references/INDEX.md` return nothing anywhere indexed. The simulated field is a SCREEN and not the test: it is Fan's exponential-decay formulation, which this project abandoned, so a mismatch confounds formulation with implementation. What it CAN falsify is the broad pattern -- deserts deep, wet tropics shallow, a coastal band -- which is driven by recharge and sea level rather than by the depth law. US and Canada are 1,405,902 of the 1,603,781 wells, and both national archives are independently obtainable, so reconstructing 88% of the compilation from primary sources is the live route [step: hydrography] |
