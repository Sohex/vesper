...
| CLIM-48 | -- | -- | wontfix, see `archive/tasks.md` |
| CLIM-56 | -- | -- | done, see `archive/tasks.md` |
| CLIM-57 | -- | -- | done, see `archive/tasks.md` |
| CLIM-58 | PARKED, and the row exists so the arithmetic is not redone. Running the model's spectral transform on the GPU is not attractive on this machine, and not for the usual reason: transfer is 128 MB a timestep at T170, which is 5.1 ms over PCIe 4.0 against a 48.9 ms transform, about a tenth. The blocker is that a consumer RTX 4090 throttles FP64 to a sixty-fourth of its FP32 rate, giving 1.29 TFLOP/s against this CPU's 1.28 with AVX-512 -- no advantage at all in the precision `config/planet.yaml` declares. `SHT_ALLOW_GPU` also documents that a plan cannot then be shared between threads, which breaks the chosen parallel design and forces the serial structure costing 140% of current runtime. And offloading the TRANSFORM alone is capped at 18.9% even at T170, because that is its whole share; a device that paid would need the physics on it too | `exoplasim/notes/shtns-viability.md` | parked, not wontfix, because two things would reopen it and neither is far-fetched: hardware with unthrottled FP64, or a decision to move the PHYSICS to the device, which is a different project decided on different evidence. FP32 on the GPU is the expensive rung of the precision ladder and CLIM-59 is the cheap one; do that first [step: rebuild_binaries] |
| CLIM-59 | Establish whether a single-precision spin-up is sound, and belongs with the restart converter because the handoff is the converter's job. MEASURED already: `compile.sh -p 4` builds FP32 -- upstream's default, which this project overrides to eight -- and it runs T170 on sixteen threads 39.69% faster [+39.63, +39.82], 4 of 4 rounds, 0.6% and 0.4% scatter. That is the largest single gain in this line of work and it needs no port and no device. The per-die working set halves from 260 MB to 130, so it pulls the same way as the 32 MB target. WHAT IS NOT ESTABLISHED is the whole risk: stability was shown over 300 steps, which is 6.2 model days and 0.043% of a 691,200-step spin-up, and single precision fails exactly where accumulation is long. Where the FP32 state LANDS is also unknown, because four-byte restart records against eight are different files to `compare_restarts.py` rather than different answers | `exoplasim/notes/shtns-viability.md`, `exoplasim/notes/restart-resolution-precision-converter.md` | open. FP32 is a SPIN-UP accelerator and never a production mode: planet.yaml declares eight-byte precision, and the converter's own contract treats a converted state as a new initial condition that must settle at the target, never as proof of equilibrium -- so FP32 has only to land near equilibrium and FP64 does the final approach and the judging, which is what dissolves the objection that FP32 cannot resolve a 0.05 W/m2 per orbit criterion. The decisive test is a full FP32 spin-up, which costs hours and is exactly what the technique claims to make cheap. Pairs with main's CLIM-52, which already has four-to-eight byte conversion in its contract [step: rebuild_binaries, baseline_run] |
| CLIM-60 | -- | -- | done, see `archive/tasks.md` |
| CLIM-61 | -- | -- | done, see `archive/tasks.md` |
| CLIM-62 | Measure what `SHT_QUICK_INIT` costs, and find a deterministic route to the alternative if it costs anything. `shtns_setup` asks for it because the default `SHT_GAUSS` BENCHMARKS SHTns's algorithm variants at startup and keeps the winner, which makes the model irreproducible -- eight runs of one binary gave four distinct restart hashes, and the check that should have caught it was asking twice. `SHT_QUICK_INIT` picks by a fixed heuristic instead, and `nm` on the resulting binary shows only `_fly` kernels: SHTns is recomputing the Legendre functions with SIMD rather than streaming stored tables. That was chosen for reproducibility and has never been measured for speed. THE PREDICTION, stated before the measurement, is that it is resolution dependent and that on-the-fly wins where it matters: stored tables are nlm*nlat/2*8 bytes, which is 0.23 MB at T42 and 0.7% of a die, but 6.05 MB at T127 and 14.36 MB at T170 -- 44.9% of the 32 MB target, before the model's own 260 MB of state is counted. So expect stored tables to win at T42, wash near T85, and lose at T170, which is the only rung where the transform is a large enough share to care | `exoplasim/notes/shtns-viability.md`, `docs/src/reference/environment.md` | open. QUICK_INIT STAYS until this is measured; determinism is not negotiable and archive CLIM-44 is this model failing to be reproducible, paid for once already. Any candidate must pass `verify_shtns_model.sh`'s four-run bit-identity arm, and four runs is itself the minimum that means anything here -- see `docs/src/practice/failure-modes.md` class 30. Three routes to evaluate and none is obviously right: `SHT_GAUSS_FLY` forces on-the-fly and is deterministic by construction, so it is the honest floor to measure against; `SHT_LOAD_SAVE_CFG` writes the tuned choice to a file and reuses it, which is deterministic only after the first run and makes the answer depend on machine state, so it needs a decision about whether that is acceptable provenance before it is worth benching; and there may be no supported way to pin stored tables without the timing pass, in which case the measurement settles the question by making the alternative unavailable rather than slower. Bench at T42 and T170 with `bench_ab.py --a-nl/--b-nl`, which compares two settings inside one binary so no compiler difference enters the comparison [step: rebuild_binaries] |
| CLIM-63 | -- | -- | wontfix, see `archive/tasks.md` |
| CLIM-64 | -- | -- | done, see `archive/tasks.md` |
| CLIM-65 | -- | -- | done, see `archive/tasks.md` |
| CLIM-66 | -- | -- | done, see `archive/tasks.md` |
| CLIM-67 | Decide whether the transform's LEVEL parallelism is costing locality, now that there is a counter for it. At T170 the SHTns path takes 11.17% of its demand fills from DRAM against legmod's 4.05%, on a quarter as many fills overall -- 107e9 against 385e9 -- so it moves far less data and misses a larger share of what it does move. The mechanism is visible in the decomposition rather than inferred: legmod is parallel over LATITUDE, so a thread touches its own band across every level and reuses it; the SHTns wrappers are parallel over LEVELS, so a thread touches the WHOLE GLOBE of each level it owns and reuses nothing between them. At T170 one level of one field is 1 MB, against a band of 64 KB | `exoplasim/notes/shtns-viability.md` | open. It is NOT a regression to fix in a hurry: the same change bought +27.64% and the extra DRAM traffic is already priced into that number, so this is about whether there is more on the table rather than about undoing anything. Two shapes worth measuring, and both keep SHTns needing a contiguous globe per level: block the level loop over latitude strips so a thread finishes a strip before moving on, or give each thread whole FIELDS rather than whole levels, which restores per-thread reuse where there are enough fields to go round -- gridpointa has five, which is fewer than sixteen threads, so that one only helps if fields and levels are collapsed into one iteration space. `profile_memory.sh` is the instrument and the fill breakdown is the measurement. CLIM-65 closed with a second, independent reason to take this one: per-thread attribution of the T170 barrier cost puts the least-waiting thread at 9.0% of its cycles and the most-waiting at 18.2%, so about six points of runtime are load imbalance, and a static schedule over NLEV=10 levels across sixteen threads is where it has to come from. Collapsing fields and levels into one iteration space would address the locality and the imbalance together. Do not judge the change on wall time alone; both effects move at once [step: rebuild_binaries] |
| CLIM-68 | Decide whether the physics should work on CACHE-RESIDENT COLUMN BLOCKS instead of whole bands, which is the NPROMA/chunk shape IFS and CAM both use. THE PRIZE IS SMALLER THAN THIS ROW WAS OPENED ON. It was opened on 31% of T170 being `__memset_avx512_unaligned_erms`, read as every physics routine clearing its own `(NHOR,NLEV)` work arrays per step; that zeroing is `-finit-real=zero` and is worth 26% on its own, which is CLIM-69, and blocking's FIRST-ORDER claim was that it moves exactly that zeroing into L1. What survives is the second-order claim, which was always the larger one. At T170 NHOR is 8192, so one work array is 655 KB A THREAD and the nine at the head of `gridpointd` are 94 MB of zeroing a step across the team. BLOCKING DOES NOT ZERO FEWER BYTES; it moves the zeroing into L1, where the physics that overwrites those lines finds them already resident, so the same instruction count stops being a bandwidth operation. The second-order win is larger than the first: `radstep`, `rainstep`, `fluxstep`, `kuo` and `icestep` each stream the whole band independently today, and a block that stays resident fuses them. It also supplies the work queue that CLIM-67 wants without the locality loss that made band ownership worth keeping, because locality then comes from the BLOCK SIZE rather than from which thread owns the data | `exoplasim/notes/shtns-viability.md` | open, and NOT gated on CLIM-59. Single precision halves every one of those zeroed bytes, but it only does so where it RUNS: the ladder uses FP32 for the spin-up rungs and ends in FP64 at the top, and the 31% was measured on an FP64 T170, so it is a cost of the run the project actually ships rather than one FP32 collects on the way past. The two are independent because they apply to different rungs. The scope is the same ~700 `(NHOR,...)` declarations and 169 `where` statements as every other physics restructuring, so if it is taken it should absorb CLIM-63, whose assign-rather-than-accumulate comes nearly free once the first writer is inside a block. The test is unusually strong for a change this size: column physics is per-column independent, so blocking MUST be bit identical and the restart sha is a check that can fail rather than a comparison that can only differ. Horizontally coupled terms and ordered reductions stay outside the blocking and are what to enumerate first [step: rebuild_binaries] |
| CLIM-69 | -- | -- | done, see `archive/tasks.md` |
| CLIM-70 | Establish what SHTns reads that it has not written, and whether the model should be running it under an unmasked FE_INVALID at all. Under `-finit-real=snan` the model traps at the first timestep inside `_an12_l`, from `spat_to_SH_fly2_l`, from `sh_gp2sp` at `shtnsmod.f90:367`, from `gridpointd` at `plasim.f90:3717`; the faulting instruction is the tail of a horizontal reduction, `vaddpd -0x20(%rsi),%ymm0,%ymm0`, and the address it reads sits 21 KB BELOW both of the wrapper's locals, inside `_an12_l`'s own frame. Three candidates are already excluded by measurement: the model does not hand SHTns an undersized buffer, `probe_shtns_nspat.c` giving nspat exactly NUGP at T21, T42, T127 and T170; it is not an unwritten m=0 imaginary part, `sh_sp2gp` assigning every element to NCSP with the m=0 imaginary set to an explicit zero; and it is in no model array | `exoplasim/notes/the-zeroing-is-an-init-flag.md` | open, and INDEPENDENT of CLIM-69 in both directions: the init flag neither causes this nor cures it, because by the time SHTns runs the stack below the Fortran frame holds leftovers from previous frames in either build. The exposure is that `-ffpe-trap=invalid` sets MXCSR for the whole PROCESS, so SHTns runs under the model's trap mask whether or not its authors assumed masked exceptions, and its padding read is safe only while the stack garbage under it is not a NaN -- which no rule in this project guarantees. What would settle it: build SHTns from `vendor/shtns-src` with sanitizers or with the read narrowed, and decide between masking INVALID around the transform call, reporting it upstream, and accepting it with the reason written down. Do not answer it by dropping `-ffpe-trap`, which is measured at under 1% and earns its place [step: rebuild_binaries] |
| CLIM-71 | Stop `gridpointd` carrying three full-globe scratch arrays per thread. `zgq`, `zmmr` and `znrho` are `(NLON,NLAT,NLEV)` locals, 10.486 MB each at T170, so 31.5 MB a thread and 503 MB across sixteen, and they are used only inside `if (nsela > 0 ...)` and the aerosol branch. `zgq` is not read at all in this configuration, its branch needing `nqspec == 0` where both the default and the bed are 1 | `exoplasim/notes/the-zeroing-is-an-init-flag.md` | open, and it OUTLIVES CLIM-69: that row stops them being zeroed, this one stops them existing on every thread's stack. 31.5 MB a thread against a 32 MB per-die working-set target is the whole budget spent on scratch for branches that mostly do not run, and it is the same shape CLIM-57 removed from `mkdheat`. The shapes to weigh are making them allocatable and allocating inside the branch, and giving them one shared copy the way the spectral partials got one. Note the gather they feed is `mpgagp` to NROOT, so a shared array is closer to what the code means than a per-thread one [step: rebuild_binaries] |
...
| CONS-13 | -- | -- | done, see `archive/tasks.md` |
| CONS-14 | -- | -- | done, see `archive/tasks.md` |
