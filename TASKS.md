...
| CLIM-2 | The first output bin of every orbit has corrupted lower-tropospheric winds, 7.5x at the bottom level, systematic across orbits and absent from the snapshot output. `carve_verdict.py`, `surface_water.py` and `export_carve_list.py` all average it into the Penman wind and inherit a 1.55x inflation, biasing open-water evaporation high and the carve verdict toward under-carving | `notes/failure-modes.md` class 14, `aeolian/README.md` | open -- find the cause in the output accumulation, and decide whether the committed lake solution needs rebuilding |
