...
| VOLC-1 | Re-examine the `melange` -> Meybeck `misc_metamorphic` mapping. It was chosen when melange was 0.00% of land; it is now 6.1% and supplies 46% of this planet's silicate CO2 drawdown | `pedology/analysis/weathering_fluxes.json` | open -- remapping it to gneiss would move implied outgassing from ~1.5x Earth to ~0.9x, so the carbon result currently rests on this one unexamined judgment |
