...
| GW-6 | Valley-to-ridge water table texture is sub-grid and stays sub-grid: Fan et al. (2013) put the well-articulated gradient at decameters to kilometres and find terrain signals dominate at local scales, against a 15.19 km region | `hydrography/notes/groundwater-scoping.md` section 5, `hydrography/notes/earth-calibration-criterion.md` | open as a DECLARED gap on GRAV-6's precedent, and now MEASURED rather than argued. GW-3 scored the solver against 70,119 Australian bores and it has no skill at all -- residual standard deviation 40.06 m against an observed 40.05 -- because a cell with no internal relief gives the water table nowhere to sit below the surface while still discharging. That is this row's limitation arriving as a number. Revisit under loop D [step: hydrography] |
...
| GW-15 | Give the water table an internal sink. Steady state with only the coast and surface seepage as exits cannot hold a water table below the surface across a dry continent, so it pins and discharges: 95.5% of Australian land and 63.0% of Vesper's | `hydrography/notes/earth-calibration-criterion.md` | open, and it is a PRECONDITION rather than the refinement GW-5 files it as. `R L^2 / 2T` with 8 mm/yr over 1,000 km needs an aquifer 600 km thick to hold the table 250 m down, so recharge in a continental interior cannot reach the sea and has nowhere to go but up. Real arid groundwater loses it to evapotranspiration from shallow tables over tens of kilometres. This is channel 2 of the scoping note's four, and until it exists every depth this solver reports is a lower bound and every seepage an upper one [step: hydrography] |
