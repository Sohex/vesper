...
| LITH-20 | The arc root is over-exposed: `granodiorite` went from absent to 7.29% of land against GLiM's intermediate plutonic 0.4%, because it is the basement under the arc belt and every arc cell whose cover erodes exposes batholith | `notes/audits/orogen-lithology.md` | open -- not a threshold to tune. An active arc is buried under its own volcanics and an extinct one is stripped, and Orogen cannot tell them apart without the resurfacing field VOLC-2 asks for |
