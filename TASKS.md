...
| --- | --- | --- | --- |
| LITH-21 | The craton archetype produces almost no gneiss (0.49% of land), so undifferentiated basement falls through to granite at 11.12% against GLiM's 5.7% acid plutonic | `notes/audits/orogen-lithology.md` | open -- and it is a CODE path to trace, not a parameter to tune. Three levers tested and none moves gneiss off 0.49%: CRATON_TECTONIC_MULT across 2.5 to 0.9, the basin factor halved then removed, and plate count 24 versus 100. Both gates were measured and both are tight, but opening either produces no gneiss, so neither is what binds. The weight is computed, thresholded, and then apparently ignored somewhere further along. Physically some craton is right -- less than Earth, since 1.881 Earth masses means vigorous convection that delaminates cratonic keels -- but delivering it needs the path debugged first |
