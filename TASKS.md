...
| CLIM-42 | Add a CH4 and N2O band to the longwave. `config/planet.yaml` says the shape of it -- "adding either means adding a band, not a key" -- and CLIM-41 measured that the term outranks every other item in the budget | `exoplasim/notes/trace-gas-band.md`, `exoplasim/analysis/trace_gas_band_model.json`, `exoplasim/analysis/co2_overlap_589.json` | open, and BLOCKED on a formulation limit rather than on work. IMPLEMENTED 2026-08-20 in `radmod.f90`: three bands on Donner and Ramanathan (1980), CH4 1306 and N2O 1285 overlapped by water vapour through the `zth2o` the routine already computes, N2O 589 overlapped by CO2 through a correlated-k fit cross-checked against Ramanathan (1976) Appendix A. Rule 4 discharged, all five binaries rebuilt and `--verify` current. THREE OF THE FOUR DECLARED TESTS PASS: the zero-abundance reduction identity is BIT-IDENTICAL on restart, output and snapshot over a 56-step segment that crosses two writes; the sign is right, OLR falling 238.759 to 238.742 at these abundances and to 238.551 at 100 ppmv; and the 100 ppmv run produces no NaN, which is the bounded-transmissivity test. **THE MAGNITUDE TEST MISSES BY ABOUT NINETY**, 0.0175 W/m2 against an offline 1.56 to 1.98, and it is NOT a coding error: the same model on the same bed gives 1.576 for a CO2 doubling, and the per-path absorptivities are individually right, 0.0119 at the full column against the doubling's 0.0115. The difference is WHERE they act -- CO2's logarithmic absorptivity adds the same amount on a thin path aloft as on the whole column, while this band model is linear in amount there and adds 0.00076 against the doubling's 0.0164. Top-of-atmosphere forcing is made aloft. Suspected cause: the band model is Lorentz-only and at the top layer's 0.025 atm returns 2.66 cm-1 where the weak-line limit gives 9.1. CESS (1973) WAS READ AND DOES NOT APPLY: its Doppler branch comes out BELOW Lorentz at every level this model has, 0.31 of it at the top layer, and the two would cross near 2 mbar against a lid of 50 -- the Doppler regime is entirely above the simulated atmosphere, and the top-layer line-centre optical depth of 53 is why. That refutes the section 4d hypothesis rather than confirming it. Calibrating the scheme against its own CO2, turning CO2 off takes OLR 238.759 to 251.043, so its WHOLE CO2 greenhouse is 12.28 W/m2 against Earth's 25 to 30 -- the scheme is uniformly weak by about two, not ninety, so the trace-gas term is still a hundred times short against that. THE BAND MODEL IS VALIDATED AGAINST A LINE LIST and is not the problem: the LMD bundle's matched pair `Earth_900ppmCO2_1mbH2O` and `Earth_900ppmCO2_900ppmCH4_1mbH2O` differ only in 900 ppm of CH4, so their transmission ratio isolates it, and Eq. (1) reproduces that HITRAN 2020 band absorptance to better than 7% at every level from 0.025 to 0.99 atm. A uniform-absorptivity probe then showed the scheme returns 2.4 W/m2 per unit of absorptivity added to `ztaucs`, LINEAR over a factor of 25 -- so the term's 0.0175 is exactly what its 0.0119 should give. **The open problem is now CO2, not the trace gases**: a CO2 doubling changes `ztaucs` by 0.011 to 0.016, which that probe says is worth 0.03 to 0.04 W/m2, and it delivers 1.576. Not the shortwave (1.598 with `CO2SWW = 0`), not another route into the flux (`zaco2` appears once in the file), not clamping (the probe is linear and `ztaucs` sits near 0.35). INSTRUMENTED, and there is NO contradiction: dumped per level, the two perturbations ARE the same change to `ztaucs` (-0.0115 to -0.0164 for the doubling against a flat -0.0164), and dumping `dftu` at the top of atmosphere directly gives +2.368 for the doubling against +2.476 for the probe, the probe slightly larger exactly as it should be, and the two stay together across all 56 radiation calls. The forty was a MEASUREMENT ARTIFACT: rank 0 holds only polar rows, so the instrumented cell was unrepresentative, and broken out by latitude the global mean hides two behaviours. In DARK rows the probe and the doubling agree, +1.024 against +0.987; in SUNLIT rows the probe gives +0.053 and the doubling +1.425. The trace-gas term follows the PROBE's pattern exactly -- +0.52 and +0.43 at the two dark polar rows and zero from 49S northward, with no response at all in 75% of cells. So the term is not weak from a wrong band model (ruled out against a line list at 7%) nor from the scheme being unable to convert absorptivity to flux (the dark rows show it can). **OPEN, and now a sharp question: why does adding absorptivity to `ztaucs` change outgoing longwave in a dark column and not in a lit one, when CO2 doing the same thing changes it in both.** The gases stay at the model default of zero and `config/planet.yaml`'s mixing ratios stay COMMENTED OUT until this closes, because declaring them while the term underdelivers by ninety would make the config assert a greenhouse effect the model does not apply [step: rebuild_binaries] |
...
| GRAV-6 | Glacial erosion carries no gravity term, and Glen's law makes ice velocity go as `g^3` (2.23x here) | `notes/audits/orogen-gravity.md` | blocked on the downscaling pass, deliberately. AMENDED: two further facts make a g^3 term precision on the wrong quantity. `iceFlow` accumulates a CELL TALLY, seeded from the glaciation index and summed downstream, where the hydraulic path was deliberately given `flowSeed = cellArea / refCellKm2` so its accumulation is a real drainage area; the two accumulators disagree about what is resolution-independent, and the same valley collects about four times the ice flow at four times the region count, carving as `iceFlow^0.6`. And per PHYS-13 the index feeding it is an Earth-calibrated latitude threshold blind to this planet's obliquity, spectrum and rotation. Fix the ice mask first. The sub-grid judgement also survives measurement: at four times the region count cells are 8.57 km against a 1-5 km valley, and reaching 1.5 km cells needs about 326 million regions, 33x the run measured. See `notes/audits/orogen-resolution.md`. The audit's recommendation was Option B, carry it as a declared gap: `iceFlow` is a heuristic accumulation rather than a Glen's-law velocity, so a rigorous g^3 on it is precision theatre. Decisive on top of that, a glacial valley is 1-5 km against a 15.19 km mesh cell, so the process is SUB-GRID and cannot be resolved here at any gravity. It belongs with the downscaling machinery, which has to persist sub-grid hypsometry anyway [step: orogen] |
