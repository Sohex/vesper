...
| CLIM-31 | `--restart-from` imports the donor run's ACCUMULATOR state, so the first output record of a seeded low-I/O run is normalised against a count that includes another run's partial window | `exoplasim/notes/first-output-bin.md` | open. Measured on a constant field, which is what makes it unambiguous: the land mask reads 0.95341 in orbit 0 bin 0 and exactly 1.0 everywhere else, a 4.7% error on something that cannot vary. It needs BOTH seeding and low I/O -- a cold-started low-I/O run reads 1.0 -- which identifies the cause as `model.configure(restartfile=...)` reading the donor whole, `seamod.f90:121-126` restoring `naccua` and `cheata` by name. NOT the defect `exoplasim-3.4.2-lowio-first-record.patch` fixed; that one was per model call and orbits 1 and 2 are clean. Scope is one bin of one orbit and every consumer that matters already drops bin 0, but it has already cost a measurement: it contaminates the low-I/O arm of CLIM-11's control pair, and a cold-started replacement is useless because a cold start sits at 268.8 K with no ice-free ocean to compare. Urgent-ish because low I/O became the default for a prepared run on 2026-08-19 and every seeded spin-up in WORKFLOW section 6 is `--restart-from`. The fix is to zero the accumulator records in the copied restart, or to have the first seeded orbit not count [step: bootstrap_run, baseline_run] |
