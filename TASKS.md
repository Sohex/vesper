...
| GW-3 | Calibrate against Earth: the same solver on Earth topography, recharge and GLHYMPS permeability, scored at OBSERVATION LOCATIONS against measured water table depth, threshold declared before scoring | `docs/src/reference/external-data.md`, `hydrography/notes/groundwater-scoping.md` section 9 | open, and now blocked on ONE input rather than on the whole compilation. Recharge is settled and the worry was backwards: Berghuijs et al. (2022) is 99% arid-weighted and excludes only permafrost, which is the right weighting for this world. US wells are fully open via the OGC API that replaced the decommissioned NWIS one, `parameter_code=72019` being depth to water below land surface. Copernicus DEM and GLHYMPS are open. **AUSTRALIA IS THE GAP AND IT IS THE REGIME THAT MATTERS**: 251,681 bore LOCATIONS are open with `landelev` and a purpose field that gives the production-well filter, but no public API serves the level VALUES -- one layer, no related table, no bulk file on data.gov.au, empty SOS2 response. They come from the Groundwater Explorer's per-state CSV or from `water@bom.gov.au` on request. No proxy is acceptable: this is the solver's only external check [step: hydrography] |
