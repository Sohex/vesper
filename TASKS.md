...
| LITH-22 | Ground the whole-rock quartz fractions in `pedogenesis.yaml` against a mineralogical source rather than QAPF read off a classification. The QAPF correction took granite from 0.45 to 0.28 and cut the felsic-mafic divergence overstatement from a factor of two to 29%, so the remaining error is in the same place; CIPW norms from average whole-rock compositions, or a modal compilation, would settle it | `pedology/config/pedogenesis.yaml`, `pedology/analysis/earth_validation.json` | open -- do it AFTER the baseline climate run and BEFORE LPJ-GUESS first runs, which is the one window where it costs no re-run. It touches 18.3% of this world's land, since the three largest classes are sedimentary and deliberately not rescaled |
