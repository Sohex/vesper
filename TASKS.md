...
| CLIM-3 | Point `carve_verdict.py`, `surface_water.py` and `export_carve_list.py` at the SNAPSHOT climatology's `spd` for the Penman wind, which is the mean of instantaneous speed at 7.488 m/s, rather than the binned product's speed of the time-mean vector. The current value is 7.732 and accidentally within 3.3%, because the corrupt bin's +55% and vector averaging's -33% nearly cancel. Do NOT fix by dropping bin 0 alone: that gives 4.997 and turns a 3% error into 33% | `notes/failure-modes.md` class 15 | open -- a 3.3% change, so no rebuild of the lake solution is required on its account |
| CLIM-4 | `ua` and `va` on SIGMA levels are u*cos(phi) and v*cos(phi), not physical winds: burn7 rescales only in the pressure-level path. Audit every consumer of those components; `sqrt(ua^2+va^2)` is wrong by a factor reaching two by 60 degrees. `spd` is physical and unaffected | `notes/failure-modes.md` class 15 | open -- found and fixed in `aeolian/scripts/build_dust.py`; other consumers not yet audited |
