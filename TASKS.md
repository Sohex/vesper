...
| LITH-23 | Ground the PRIMARY CLAY fractions in `pedogenesis.yaml`, which set the texture of freshly produced regolith and are what actually controls the felsic-mafic contrast. Wants a weathering chronosequence with particle-size data on young basalt and granite surfaces, not mineralogy. Lowering mafic primary clay is the direction that fixes the divergence and the bias together | `pedology/config/pedogenesis.yaml`, `pedology/analysis/earth_validation.json` | open -- before LPJ-GUESS first runs, since it derives water capacity from sand and clay directly |
