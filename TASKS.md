...
| --- | --- | --- | --- |
| MIN-1 | Emit an economic-minerals overlay from Orogen for the tectonic and magmatic deposit types: porphyry, orogenic gold, VMS, magmatic Ni-Cu-PGE, podiform chromite, kimberlite | `notes/economic-minerals.md` | open -- a SEPARATE field, never a `substrate_class` value and never an erodibility modifier, or it re-enters the climate path. First consumer of `hotspot`, `r_tectonicActivity` and `r_t_craton`, which nothing currently reads |
...
| MIN-3 | Decide prospectivity versus discrete occurrence, and the sampling rule if both | `notes/economic-minerals.md` | open -- a continuous favourability field is honest about what the model knows; discrete deposits are what a culture finds. Likely both, but the sampling rule should be chosen rather than invented in passing |
