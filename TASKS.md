...
| LITH-20 | The arc root is over-exposed: `granodiorite` reached 7.29% of land against GLiM's intermediate plutonic 0.4%, and 59.5% of the arc belt is stripped to basement | `notes/audits/orogen-lithology.md` | open -- and it is a COVER THICKNESS question, not a missing field. `LITHO_COVER_ARC_KM` is 1.0, thinner than a LIP's 1.2 and far thinner than basin fill's 3.0, so an arc veneer that should be continuously resupplied is stripped by ordinary erosion instead. Resurfacing is representable as a cover that erosion cannot outpace. Testable in one build; needs a build to measure |
