...
| HYD-8 | Replace the discharge-only retain mapping with a depth-normalised one, `retain = clip(1 - C * erodibility * Q**0.5 / depth_at_spill, 0, 1)`, and declare `C`. The current rule gives `retain = 0` to all 29 of Earth's sill-impounded through-flowing basins, Tanganyika and Malawi included, because it never asks how deep the basin is -- and depth at spill spans 14 to 1,420 m on this build and is in the export already | `hydrography/notes/retain-fraction.md` | open -- the fix is not in doubt, `C` is. Earth bounds it above at 0.54 m per (m3/s)^0.5 over the lifetimes of its own lakes, which is far too small for a landscape relaxation window; the marginal class is 1534 basins there and 275 at C = 50. NEEDS A DECISION on the window before the carve list is handed to Orogen |
