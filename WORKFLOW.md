...
**The words are in `CLAUDE.md` under "Vocabulary", and they are not repeated
here.** `generation`, `build`, `bootstrap run`, `baseline run`, `commissioning`,
`re-commissioning`, `re-run the baseline` and `iteration` all have exact meanings
and two of them have been misread already. They live in the file that is loaded
before anyone starts talking, because that is where both misreadings happened.

What belongs here is the cost, because that is pipeline reasoning rather than
language, and because the cost is what makes the distinction worth drawing.
