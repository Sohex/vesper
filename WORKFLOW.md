...
Every artifact this project keeps, the step that generates it, and what reads it.
**An artifact absent from this table does not exist**, per the rule at the top.
Products that exist only to be looked at once -- validations, sweeps, figures --
are listed too, marked `one-off`, because the distinction that matters is whether
something downstream depends on them, not whether they were interesting.

| artifact | generated by | read by |
| --- | --- | --- |
| `source/<build>/` | World Orogen (`vendor/orogen`) | everything; `lib/orogen.py` is the reader |
| `config/planet.yaml` | hand-edited; the only such file | every component |
| `exoplasim/inputs/stellarspectra/k25v{,_hr}.dat` | `exoplasim/scripts/build_stellar_spectrum.py` | the model, `lib/stellar.py`, `dust_optics.py` |
| `exoplasim/plasim/bin/most_plasim_*.x`, `binary_manifest.json` | `exoplasim/scripts/rebuild_binaries.py` | every run; `check_consistency.py` |
| `exoplasim/inputs/exoplasim_cycle_t42/`, `cycle_binary_manifest.json` | `exoplasim/scripts/build_star_cycle_exoplasim.sh` | `run_stellar_cycle.py` |
| `hydrography/data/<build>/regions.nc`, `basins.nc`, `coupling_*.nc`, `hydrography_report.json` | `hydrography/scripts/build_hydrography.py` | carve verdict, pedology, minerals, aeolian, surface water |
| surface codes 129, 172 + `boundary_conditions_report.json` | `exoplasim/scripts/build_boundary_conditions.py` | the model |
| surface codes 174-176, 212 + `albedo_report.json` | `exoplasim/scripts/build_surface_albedo.py` | the model |
| surface code 173 + `roughness_t42_report.json` | `exoplasim/scripts/build_surface_roughness.py` | the model |
| surface code 229 + provenance | `exoplasim/scripts/build_surface_soil_water.py` | the model |
| surface code 1811 + provenance | `exoplasim/scripts/build_surface_dust.py` | the model, when `ndustrad = 1` |
| `exoplasim/data/dust/vesper_dust_aerosol.dat` | `exoplasim/scripts/dust_aerofile.py` | the model's `aerofile` |
| `exoplasim/runs/<uuid>/` + `run_manifest.json` | `run_exoplasim.py`, `continue_exoplasim.py`, `run_stellar_cycle.py` | everything below the climate |
| `exoplasim/runs/INDEX.json` | `exoplasim/scripts/index_runs.py` | `world_state.py`; the only record of what a run was |
| `exoplasim/analysis/energy/<run>_state_energy_*.json` | `exoplasim/scripts/close_state_energy.py` | `assess_convergence.py`, which PASSES on its storage term |
| `exoplasim/analysis/convergence/<run>_convergence.json` | `exoplasim/scripts/assess_convergence.py` | the run's own status; nothing may treat a run as settled without it |
| `*_regular_climatology.nc`, `*_snapshot_climatology.nc`, `*_climate_series.json` | `exoplasim/scripts/build_climatology.py` | hydrography, pedology, aeolian, biosphere |
| `*_climate_report.json`, `*_classification.nc`, climate figures | `exoplasim/scripts/analyze_climatology.py` | `world_state.py`; the Koppen and biome products |
| `hydrography/data/<build>/surface_water.nc` + report | `hydrography/scripts/surface_water.py` | albedo compositing, aeolian, pedology |
| `hydrography/analysis/carve_verdict.json` | `hydrography/scripts/carve_verdict.py` | `export_carve_list.py`, `world_state.py`, the error budget |
| `carve_list.txt`, `carve_list.json` | `hydrography/scripts/export_carve_list.py` | World Orogen. THE ONE IRREVERSIBLE ARTIFACT |
| `aeolian/analysis/dust_baseline.{nc,json}` | `aeolian/scripts/build_dust.py` | `dust_forcing.py`, `build_surface_dust.py`, pedology loess and phosphorus |
| `analysis/dust_optics.json` | `exoplasim/scripts/dust_optics.py` | `dust_aerofile.py`, `dust_forcing.py` |
| `analysis/dust_forcing.json`, `analysis/dust_surface_forcing.nc` | `exoplasim/scripts/dust_forcing.py` | `carve_verdict.py --dust`, the error budget |
| `analysis/shortwave_band_weights.json` | `exoplasim/scripts/shortwave_band_weights.py` | PHYS-1's `h2osww`; the derivation behind it |
| `pedology/data/<build>/` soil + `soil_report.json` | `pedology/scripts/build_soil.py` | code 229, biosphere, minerals, derived surfaces |
| `pedology/analysis/weathering_fluxes.json` | `pedology/scripts/weathering_fluxes.py` | the CO2 requirement, `thermostat_efficiency.py`, VOLC-3 |
| `pedology/analysis/brine_paths.json` | `pedology/scripts/brine_paths.py` | `build_surface_classes.py`, `build_downstream_prospectivity.py` |
| `pedology/analysis/surface_classes.nc` + report | `pedology/scripts/build_surface_classes.py` | minerals downstream, maps |
| `pedology/analysis/phosphorus_budget.json` | `pedology/scripts/phosphorus_budget.py` | the biosphere's C-N-P fork |
| `pedology/analysis/thermostat_efficiency.json` | `pedology/scripts/thermostat_efficiency.py` | the carbon-cycle bound in section 4 |
| `biosphere/generated/` header, PFTs, driver + provenance | `build_vesper_header.py`, `build_vesper_pfts.py`, `build_lpj_driver.py` | `run_lpj_guess.py` |
| `biosphere/runs/<uuid>/*.out` | `biosphere/scripts/run_lpj_guess.py` | `build_soil.py --soil-carbon`, `build_surface_albedo.py --mode modelled` |
| `minerals/data/prospectivity.nc` + report | `minerals/scripts/build_prospectivity.py` | terminal; maps |
| `minerals/data/downstream_prospectivity.nc` + report | `minerals/scripts/build_downstream_prospectivity.py` | terminal; maps |
| `analysis/error_budget.json` | `scripts/error_budget.py` | the ranking of what to refine next |
| `world_state.json` | `scripts/world_state.py` | every reader who wants a current value |
| `archive/runs/`, `archive/builds/` | `scripts/archive_runs.py`, `scripts/archive_builds.py` | identity of things whose payload is gone |
| `maps/build/basemap_*.png` + provenance | `maps/build_basemap.py` | terminal; the eye |
| `maps/build/projections_*` + provenance | `maps/render_projections.py` | terminal; the eye |

One-off, depended on by nothing, and kept because the argument is worth reading:
`validate_lake_solver.py`, `pedology/validate_against_earth.py`,
`biosphere/score_prediction.py`, `compare_flux_sweep.py`,
`compare_albedo_bracket.py`, `analyze_stellar_cycles.py`,
`aeolian/dust_runoff_sensitivity.py`, `pedology/probe_runoff_response.py`,
`hydrography/lake_balance.py --sweep`, `exoplasim/analyze_smoke.py`,
`exoplasim/mie_dust.py`. `extract_high_cadence_wind.py` and
`finalize_existing_segment.py` are run repair and diagnostics, not steps.
`sra.py`, `solute_routing.py`, `drainage.py` and `lib/` are modules, not steps.
