...
| equilibrium acceptance | the trending-cell limit is MEASURED, not declared: each field's limit is the largest share of its occupied cells that any detrended window of the run's own pre-acceptance record flags, and the retained record length sets the false-refusal rate at 1/(N+1). One declared fraction could not serve fields whose null spans 0.001 to 0.40, and the earlier 0.25 sat below the rate the test flags with no trend present. `notes/equilibrium-trend-null.md` has the measurement; `scripts/derive_trend_null.py` re-takes it |
