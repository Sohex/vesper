...
| run lengths | DERIVED, not typed. `lib/run_lengths.py` now carries a second timescale pair for this model, in complete forcing cycles and never interchangeable with the climate pair in orbits: a retained record of at least ten times the memory time (125.3 cycles, so 1253) and a spin-up long enough that the approach it leaves drifts the record by less than the acceptance contract's own tolerance (846.8 cycles of relaxation, so 2318). `build_vesper_pfts.py` reads the spin-up floor instead of rescaling Earth's 500-year convention, which gave 998. Both are FLOORS: 123 of 128 fields' approach is not measurable on a 1000-cycle record. The whole run is 3571 cycles, about 55 minutes at npatch 5 |
