...
- `scripts/fire_nitrogen_bounds.py` -- one-off: two conservation bounds on a completed run's fire nitrogen flux, each with a right answer. STOCK: in one simulated year a gridcell cannot lose more nitrogen to fire than the nitrogen it holds. RATE: over the retained record the cell is at equilibrium, so its losses equal its inputs, and fire is one of at least three loss pathways, so mean fire loss must be strictly below mean supply. Both are derived from the INPUT side, which is what lets them judge a run whose fire distribution has already been looked at -- neither is written in terms of the distribution it judges. The kgN/m2-to-kgN/ha factor between `npool.out` and the two flux tables is READ from `M2_PER_HA` in the model source rather than written here, because the whole correctness of the check is that one conversion and the comment at `commonoutput.cpp:196` invites exactly the wrong reading of it. `--strict` exits non-zero on a violation; without it the module reports, because a violation is a finding about a run rather than about the tree. `notes/fire-nitrogen-range.md` carries the first application and what it settled. Registered under `one_offs`; it generates nothing.
...
- `scripts/polar_water_timing.py` -- one-off: asks WHEN the simulated polar cap's water crosses the surface, which the capture ceiling in `notes/polar-cover-cold-filter-and-capture.md` assumed rather than measured. The ceiling adds two terms of the run's own water balance as recoverable by an adapted flora, and a term leaving under frozen ground is recoverable by nothing, so the bound would be an overstatement rather than a ceiling. Forcing comes from the driver LPJ-GUESS read and not from the climatology re-read on another calendar, so the growing-season window is the one the run integrated. Three controls: the two monthly tables must sum to the annual figures the note published, and the season length falls out and must match. It refuses rather than reports when a control misses, states the verdict rule before the numbers, and says which term it cannot time because `tot_runoff.out` is annual. It generates nothing.
- `scripts/polar_capture_efficiency.py` -- one-off: asks whether the simulated polar cap is INEFFICIENT at taking water or simply carries no leaf, which decides whether the capture-side traits `notes/polar-cover-cold-filter-and-capture.md` proposed for sizing are aimed at anything. It ranks each band's capture per unit leaf area against the distribution over all cells, and refuses to read that ratio as a physical constant: the same table shows a sixteen-fold spread, so a Beer's law in leaf area alone is reported as refuted and only the ranking is used. Two controls that can fail, the band's reconstructed precipitation and its capture share against the figures the note published. It generates nothing.
- `scripts/pft_exclusion_sweep.py` -- one-off: asks over the whole planet which plant functional types each gridcell bars and by which declared limit, and whether the cover that remains runs inside the temperature band it declares. It keeps the two signatures apart on purpose: a barred type is evidence of nothing, since boreal types are correctly absent from the tropics and the ground is full there, so the finding is the cover-weighted share of growing season the RESIDENT types spend above their own `pstemp_high` and `pstemp_max`. It also keeps SURVIVAL apart from ESTABLISHMENT: only `tcmin_surv` kills, so it alone is the control, while a type barred from recruiting legitimately persists. Its calendar tolerance is declared, because the driver's 15.083-day intervals cannot align with the model's whole-day months. `notes/underoccupied-niches.md` is the finding. It generates nothing.
