...
def render(constants: dict) -> str:
    """The header text, which carries no generation date.

    THE HEADER'S HASH IS WHAT DECIDES WHETHER THE MODEL BINARY IS STALE.
    `VESPER_YEAR_LENGTH_DAYS` sizes arrays, so `build_lpj_guess.py --verify`
    refuses a binary built from a header that has since changed, and it decides
    that on the file's hash. A generation timestamp in the text moves that hash
    for a regeneration that changed nothing it declares, which condemns a
    correct binary and costs a rebuild to restore a byte-identical model. The
    date belongs in the provenance JSON beside it, which nothing compiles and
    no staleness decision reads. This is the same failure the `source_config`
    comment below records one level down, where a hash over the whole config
    moved for an edited comment.
    """
