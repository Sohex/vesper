...
```bash
python exoplasim/scripts/build_boundary_conditions.py   # topography + binary/fractional land support
...
| `verify_model_compiles.py` | does the model SOURCE compile: `gfortran -fsyntax-only` over every translation unit `plasim/CMakeLists.txt` names, sorted into `use` order and run under the flag line `config/planet.yaml` declares, `-fdefault-real-8` included. It builds no binary and answers nothing about whether one is current, which is rule 4's separate question. Run it after a Fortran edit, before paying for a build. It is the PRE-BUILD tier of CLAUDE.md rule 8 and `scripts/smoke_test.py` does not run it: a process per translation unit is not a per-commit cost, and what the fast gate keeps of the Fortran is the dropped-continuation text check, which catches the one class that has cost a build and catches it in the commit that makes it |
| `verify_build_path_independence.py` | is the executable a function of the model source, or of where the checkout sits? Stages the driver, `config/planet.yaml` and `plasim/src` into two roots that differ in depth and in length, shows the two stagings byte-identical so a difference can only be the path, builds each through `build_model.py` itself and compares the two shas. `binary_manifest.json` identifies a binary BY its sha, so a sha that moves with the checkout is a binary no other checkout can register -- which is what stopped a worktree running anything it had compiled. Two builds at the rung asked for, `--no-publish` on both, and each arm's run directory is inside its own staging, so the registry is unreachable from it |
| `prefix_map_launcher.py` | not a gate and not run by hand: the compiler launcher `build_model.py` puts in front of every Fortran compile. CMake preprocesses each source into the build directory so it can scan the module graph, which puts the checkout's absolute path into the cpp line markers, and `gcc(1)` says `-ffile-prefix-map` does not remap a directory named by a directive; gfortran then stores that path in `.rodata` beside every I/O statement, where it survives with no debug flags at all. This applies the maps already on the compile line to those markers. The maps stay declared in `plasim/CMakeLists.txt` and are not restated here |
| `lint_masked_domains.py` | which domain-sensitive intrinsics and divisions sit inside a masked `where` block in `plasim/src`. A `where` masks the ASSIGNMENT and not the evaluation, so the compiler may evaluate a right-hand side on lanes the mask excludes and the declared `-ffpe-trap` turns a discarded lane into SIGFPE; that is world-bhs, and it is why the flag line is `-O2` and not `-O3`. A text parse, no build and no run. It OVER-reports by construction and says so, and the sites no floor can be argued for carry their reason in its `CLASSIFIED` table, keyed on the argument text so a changed argument is reported again. Exit 1 on anything neither guarded nor classified; `--all` prints the whole population, `--kind divide` the divisions. Divisions are reported in all three spellings -- a parenthesised expression, a bare name, and a name with a subscript or an argument list -- because the trap does not care how the divisor was written. `--kind any` is the default and is the gate: the intrinsics, the divisions and the merges together, every one of them either guarded or classified |
...
| `lint_masked_locals.py` | which procedure locals in `plasim/src` have ALL their definitions inside a `where`. A `where` selects which elements the assignment STORES and not which the compiler EVALUATES, so an automatic local with no initialiser, written only inside a mask and then read, is read on every discarded lane as whatever the stack held, and the declared `-ffpe-trap` turns that into SIGFPE; where the read sits under a different mask or none, the indeterminate lane is stored rather than discarded. world-d016 found the class in `tands`, `mktsoil` and `mkdca` and left it unenumerated. Adjacent to `lint_implicit_save.py` and its exact complement -- that pass fires on the PRESENCE of an initialiser, this one on its absence -- and the module argues why they are not one pass. A text parse, no build and no run, over-reporting in five declared directions and blind in two. It answers twenty reduced fixtures before it reports on the tree, and its positive control is the pre-d016 source. Exit 1 on anything with neither a fix nor a `CLASSIFIED` row; `--all` prints the whole population, `--file` restricts it to one source file. Its DEFERRED table, which reports a file without gating it, is empty |
| `lint_diag_writes.py` | which writes to the shared diagnostics unit a thread other than 0 can reach. `nud` is unit 6 and `opendiag` opens it once under a root guard, so it is the diagnostics file for the WHOLE thread team and `write(*,...)` lands there too; the run is one `!$omp parallel` region, so an unguarded site is NPRO interleaved copies in `plasim_diag` and the file stops being the same twice. It permits four answers and nothing else: an enclosing `mypid == NROOT`, an enclosing `npro == 1`, an enclosing `!$omp critical`, or an enclosing procedure the call graph shows is reached only through those. A text parse, no build and no run, over-reporting the same way the two above do; it answers reduced fixtures on every invocation before it reports on the tree, because a gate whose only evidence is a clean tree cannot tell a clean tree from a broken pass. Exit 1 on anything unanswered, and `scripts/smoke_test.py` runs it; `--all` prints every site with its verdict, `--procs` the call-graph verdict per procedure |
| `lint_diag_arrays.py` | whether the four optional diagnostic blocks still have code bands of their own, and whether the model still refuses to fill more arrays than it allocates. `outdiag` and `snapshotdiag` number the blocks off the LOOP INDEX into the same unit the ordinary output goes to; off the bases they carried, one 2-D spectral array wrote onto the ecliptic longitude's code and `pyburn` reshaped the joined records by the first header it saw. The bases moved into free space (`plasimmod.f90`: 700, 800, 900, 1000, 99 codes each), which is a boundary only while the bands stay disjoint from each other and clear of every code anything else writes -- and this is what holds that against the source. It also derives from the writes how many arrays `NDIAGGP` and `NDIAGSP` fill, which is what `plasim.f90:check_diagnostic_blocks` demands of `NDIAGGP3D` and `NDIAGSP3D`: those are different namelist keys and nothing else ties the switch to the allocation. A text parse, no build and no run; it answers reduced fixtures on every invocation before it reports on the tree. Exit 1 on any problem; `--all` prints every fact with its verdict. world-2v9z |
| `conversion_time_stability.py` | what timestep `model.conversion_time_level` is stable at, and it is not the explicit gravity-wave limit `plasim.f90`'s NCONVTIME guard used to test. The term puts the reference conversion's divergence half back on the divergence at t, and `sdt - sd` is the second time difference: O(dt^2) for a smooth mode and exactly `-2 sd` for the LEAPFROG COMPUTATIONAL MODE, so the term feeds that mode, the Robert-Asselin filter is the only thing damping it, and the boundary is a function of PNU -- at PNU = 0 there is none at any step. It rebuilds the model's own linearised adiabatic step from `config/planet.yaml` through `initpm`'s, `initsi`'s and `makebm`'s arithmetic and takes the spectral radius of the one-step map exactly. The MODEL cannot solve an eigenvalue problem at startup, so its guard iterates and runs a CONTROL ARM to measure the iteration's own bias on a neutral map; `verdict` reproduces that and `--against <run>/plasim_diag` compares it arm for arm with what the model printed, which is the check with a right answer. No build and no run; writes `exoplasim/analysis/conversion_time_stability.json` over the ladder, and `--rung <rung> --sweep <steps>` writes the per-timestep table in `conversion_time_stability_sweep.json` that `exoplasim/notes/convdecomp-reproducibility.md` cites. world-bt3b |
| `conversion_time_arms.py` | the same boundary against the FULL model rather than its linearisation: arms either side of it, run long enough for the predicted growth to reach the range the declared `-ffpe-trap=overflow` refuses. Termination is read from `Abort_Message` and from the exit status SEPARATELY, never from the status alone -- `stability_check` writes that file and executes a bare Fortran `stop`, which exits 0 -- and there is a third outcome, the NCONVTIME guard's own refusal, which on the exit status is also indistinguishable from a completed run. Control arms at the same steps with the term off are required, because an arm that dies says the configuration died and not that the term killed it. It confirms a RATE and not a sign: each death is bracketed by the report cadence, the predicted rate turns that bracket into an amplification, and one exponential accounting for every death means those intervals share a value. Runs the model, so it needs the host lock; writes no run and registers nothing, because these are beds. world-bt3b |
...
| `profile_transforms.sh` | sample the model with `perf` over a bed, one process and one sample file, launched the way production launches it |
...
| `bench_ab.py` | interleaved A/B of any number of executables on one bed, paired per round, with the starting arm rotated by round. `--factorial` takes four arms as base, factor A, factor B, both, and reports each factor's gain within each level of the other beside the excess over what multiplying the two single-factor gains predicts. `--counter EVENT` compares a `perf` hardware counter instead of the clock, which is what a contended host leaves usable; `--outlier-factor` drops a round WHOLE when any arm in it exceeds that multiple of its own median, and always prints what it removed |
| `patch_unmasked_absorptance.py` | switches `radmod.f90` between the masked and the unmasked form of `lwr`'s four absorptance selections, `--apply` / `--revert` / `--check`. The unmasked form is MEASURED and NOT ADOPTED, so the tree holds the masked one and this holds the arm |
| `probe_unmasked_absorptance.f90` | the numerics half of that arm: it evaluates all four absorptances both ways over path amounts straddling every threshold, under the model's own `-ffpe-trap`, and asserts element equality. Standalone, so it needs no bed and no binary |
...
| `verify_banded_transform.sh` | the model's own scatter, weight matrices, banded partial sums, cross-thread reduction and `dv2uv` against a reference computed outside the model, at bounds declared before the arms run, with four controls: the quadrature weight dropped from the analysis, the bands slid one row, the scatter handing every thread the same latitudes, and `dv2uv`'s planetary vorticity correction shifted by the thread's own index. Refuses fewer than two threads, because both band controls are no-ops at one |
...
| `verify_canopy_snow.sh` | the canopy snow store against the two things about it that have right answers. The REDUCTION is bitwise: at a non-positive `forpai` there is no capacity, so the throughfall has to be the snowfall to the last bit and the ground pack's tendency has to be the expression it already was. The MASS BALANCE is an identity accumulated outside the routine over a driven season with a thaw in the middle of it -- the snow that fell equals the throughfall plus the change in the store -- so an arithmetic path that loses or invents mass fails here rather than as a surface water balance that will not close. And four limits: the load stops at its capacity, never goes negative, empties on the step the air goes above freezing, and approaches `c S_f tau` |
| `verify_canopy_snow.f90` | its driver. `cansnowmod` reads no model state, so this compiles it alone: no grid, no namelist, no build tree. At `-fdefault-real-8`, for the same reason `verify_snow_mask.f90` is |
| `verify_absent_restart_record.sh` | what happens to a caller's array when the restart does not carry the record it asked for. `get_restart_array` leaves its argument alone on a miss, which invites a sentinel; `mpgetgp` scatters a buffer of its own over that sentinel whatever the lookup did, and `landmod` decided a rebuild on the result. The control is the PRE-FIX reader shape and it must scatter a NaN and make `ALL(p < 0.0)` come back false -- at `-finit-real=snan`, because at the shipped flag line the same read returns ordinary stack that no instrument can tell from a number. `has_restart_array` and `mpgetgp_found` are what it certifies |
| `verify_absent_restart_record.f90` | its driver. `restartmod` reads no model state, so this compiles it alone: no grid, no namelist, no build tree. `mpscgp` needs the whole of `pumamod`, so the two reader shapes are TRANSCRIBED with the copy it performs on a one-thread team; keep them in step with `mpimod_omp.f90` |
...
| `assess_convergence.py` | TWO verdicts, for two questions. Spin-up convergence against the predeclared criteria, over the last `--window` PRODUCTION orbits, reporting what that window can RESOLVE beside every verdict, the window each criterion would need at this run's own scatter and autocorrelation, and WHICH ESTIMATOR the offset criterion was taken on with the reason the other was declined. And, for a run the manifest says began from a conversion, RECONVERGENCE: whether the transient that conversion introduced has decayed, fitted at the relaxation time the planet implies over the whole post-conversion record and held to the same allowance. A run too short for the convergence window gets the reconvergence verdict alone, with the convergence criteria recorded as unasked rather than failed; a longer one gets both as separate keys, and a reconvergence verdict is never a convergence verdict. `--through` assesses a prefix and writes into neither the run's manifest nor the run's own report |
| `check_relaxation_ceiling.py` | reads every convergence report and asks whether the DERIVED relaxation time bounds the FITTED one. The offset criterion's fallback holds the derived time fixed, which is conservative only while it is a ceiling; a run whose exponential fit has grip is evidence and a settled run is not |
...
| `verify_high_cadence_rescue.py` | does a high-cadence segment's raw stream survive a failed conversion? On fixtures: the raw is found and stamped whether the model left it at the run root or `_crash()` moved it into a `<run>_crashed/` sibling, the sample count the check demands is counted off `plasim.f90`'s own write condition rather than asserted, and a file carrying the regular output's twelve bins under the high-cadence name is REFUSED -- that is ExoPlaSim's integritycheck fallback re-running pyburn under `example.nl`, which is what made run_67323a923013's failure look like a success. No model, no process, milliseconds |
...
| `ocean_tier_gate.py` | the adopted ocean tier declared and checked against the source the model compiles: `exoplasim/config/ocean_tier.yaml` against `oceanmod.f90`, `icemod.f90` and `seamod.f90`. Fails on a quantity two modules act on that is no longer stated once and handed on, on a constant whose declaration line the source no longer runs or whose value is not what that line's own initialiser evaluates to, on a constant whose declared reachability its namelist block contradicts, on the sea-ice albedo ramp leaving the hull of its own endpoints, on a line the tier's argument rests on, and on a fail-closed guard that has gone. Nineteen fixtures, eighteen built to be wrong in a named way; `--strict` refuses on the Earth constants of the modelled sea that are still unsourced. Nothing here is verified by execution |
| `verify_ocean_flux_channel.py` | verifies the prescribed ocean heat-transport channel, surface code 903 under `oceanmod_nl` `nfluko = 1`, end to end on a field whose answer is stated before the run. Four arms, each isolating one path: `channel` on a field constant in time and an ice-free ocean, `monthly` on a separable time-varying field that exercises `getflxco`'s interpolation alone, and `ice` and `ice_clim` on a declared cold start that grows sea ice and reaches `addfc`'s branch (c) and branch (b). `stage --arm` writes the fields and the prediction into a run directory; `selftest` synthesises a stream satisfying every identity and breaks each criterion in one named way, so the instrument is checked before the model is run; `check` answers each criterion against that run's `ocean_output` stream and exits non-zero if one fails |
...
| `derive_design_flux.py` | codifies `docs/src/pipeline/state.md` section 5b's flux choice: declared comfort-band thresholds scored per candidate flux from two converged points, with the humidity-coupled variant beside the dry score, a swept bracket on each comfort threshold, and refusals on an undeclared cold-extreme cap and on a winner at either end of the candidate range; writes `analysis/design_flux.json`, whose `basis` is `derived` or `chosen`. `--chosen FLUX --evidence notes/...` records a flux the derivation could not produce, and writes only after running the derivation and catching a refusal whose ground is listed in `CHOOSABLE_REFUSALS` -- a derivation that answers is adopted, not overridden, and a refusal fixed by supplying an input is not a ground |
| `verify_design_flux_projection.py` | the design-flux band projection on synthetic bands whose answer is known before it runs. Four properties: the line passes through BOTH of its own measured points, a negative warm-season response is projected rather than refused, a positive one does not certify a projection whose seasonal range closes inside the candidate set, and the validity window is one interval. The anchor case carries the superseded ratio-through-a-foreign-slope arithmetic as a control and requires it to fail, so the identity is known to have teeth rather than assumed to. No model, no run, no artifact |
...
| `cloud_band_weight.py` | the CLOUD half of the same correction: Mie over liquid water's k(lambda) to a co-albedo, flux-weighted over range 2 against a 5772 K Sun, for `model.cloud_absorption_scale`, which reaches the model as `cloudabs` and scales the tabulated range-2 co-albedo -- the only shortwave cloud absorption the computed-cloud branch `nswrcl = 1` reads. PHYS-11, clim-68, world-f9ig |
| `cloud_optical_depth_bracket.py` | prices world-jgen's band-1 cloud optical depth correction over a real field: the model's own per-layer cloud water path, its own total cloud cover with random overlap, its own zenith geometry integrated over the day, and the band-1 surface albedo through Stephens Eq. (12). Reports a per-layer table and a bracket over where in the column the cover sits, in W/m2 and in kelvin through `lib/sensitivity.py`. Writes `analysis/cloud_optical_depth_bracket.json`. Only a run settles it |
| `socrates_cost_bench.sh`, `socrates_cost_bench.f90` | builds the CLIM-61 candidate OUTSIDE the tree and a single-column driver for it, at `config/planet.yaml`'s own production flag line less `-ffpe-trap`, so what is compared is the code and not the compiler. Outside because `references/socrates` is read-only reference and in a worktree it is a directory symlink into the main checkout. It refuses to hand over a driver whose `examples/runes` key output does not reproduce SOCRATES's own reference to 1e-7. Point `SOCRATES_BENCH` at what it builds. CLIM-61 |
| `radiation_cost_per_column.py` | what ONE RADIATION CALL COSTS PER COLUMN, in CPU-seconds, for the model's broadband scheme and for a band-resolved candidate on the same host in the same unit. Runs the model on a profiling bed under `perf` at two step counts and takes the difference, so the startup cancels rather than being assumed small, and attributes the radiation share by symbol. The candidate arm needs a SOCRATES driver built outside the tree and named in `SOCRATES_BENCH`, and reports itself unavailable rather than guessing when there is none. Writes `analysis/radiation_cost_per_column.json`. CLIM-61 |
| `stephens_tables_vs_fits.py` | carries Stephens (1978) Table 2 and Stephens et al. (1984) Table 1, reads `radmod`'s Fortran port of the 1984 tables back against them, checks `swr`'s conservative co-albedo floor against Stephens Eq. (1), and measures what adopting the tables was worth against the analytic fits they replaced. Three checks that fail on a slipped digit; `smoke_test` runs them. Writes `analysis/stephens_tables_vs_fits.json`. world-f9ig |
| `swr_divisor_domain.py` | whether `swr`'s thirteen unbounded divisors can reach zero on this planet's configuration, evaluated over the model's own absorber ranges rather than argued. The nine clear-sky band transmissivities `1 - A(u)/zsolar_b` and the four adding-method products `1 - R_above*R_below`, each with its supremum, its crossing amount where one exists, the model's own maximum, and the margin between them. Five checks with a right answer stated in advance. Writes nothing: what it produces is a bound, and the bound lives in `notes/audits/masked-where-blocks.md` and in `lint_masked_domains.py`'s `CLASSIFIED` rows. world-2223 |
| `lw_continuum_bracket.py` | how large `th2oc`, the whole of the modelled longwave water vapour continuum, can be. Rebuilds the PRESSURE-WEIGHTED water path `lwr` applies it to from a climatology's own `hus` and `ps` on the model's own sigma grid, then bounds the coefficient by the requirement that a term standing in for the window cannot absorb more than the window's share of the emitted Planck flux. Reports the path range, what the inherited value already claims, and the ceiling across the modelled emitting temperatures. Writes nothing. world-2esd |
| `spectral_tail.py` | asks whether the damping is absorbing the cascade at the truncation or reaching down into the resolved scales: fits the flow's own inertial range in zonal wavenumber and reports where the spectrum leaves it. Criteria are fixed in the script. The tail slope beside them is fitted only over wavenumbers standing clear of the measured roundoff floor, and names the band. Writes `analysis/spectral_tail.json` |
| `filter_spectral_cost.py` | prices the physics filter against the resolved spectrum and says whether the instrument can see a change in `filter_kappa` at all: the bite point, the depth below the run's own inertial range at 0.8 of the truncation, the eddy kinetic energy removed, the scatter of each across disjoint windows, and what a stated factor in kappa is worth in those units. Criteria fixed in the script. `--compare` reports arms side by side against the pooled scatter. Writes `analysis/filter_spectral_cost.json` |
| `measure_eddy_wind.py` | measures the eddy RMS wind `config/planet.yaml`'s hyperdiffusion rule divides by, on a run's INSTANTANEOUS snapshot stream: departure from the zonal mean at the same instant, level and row, area-weighted by the model's Gaussian row weights and mass-weighted by `dsigma`. Reports the top level and the jet-inclusive total beside it, and the same recipe on the TIME-MEAN stream, which carries the stationary eddies only and is a different and smaller quantity. Criteria fixed in the script: the sampling scatter through `lib/autocorrelation.py` and the share of eddy variance above 0.7 of the truncation, which is what says whether one rung's measurement transfers to the ladder. Writes `analysis/eddy-wind/<run>.json` |
...
adiabatic step, once with the term and once without, and refuses when the
difference exceeds what the control arm shows the iteration can be wrong by;
`conversion_time_stability.py` computes the same map, and the exact boundary
beside it, with no build and no run. `world-bt3b`.
