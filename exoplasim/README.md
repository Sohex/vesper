...
| `stephens_tables_vs_fits.py` | carries Stephens (1978) Table 2 and Stephens et al. (1984) Table 1 and asks how far `radmod`'s own `tswr1`, `tswr2` and `tswr3` fits sit from them at the optical depths this model's cloud layers reach. The transcription is checked against the 1984 paper's own claims about which of its tables changed, which is a check that fails on a slipped digit. Writes `analysis/stephens_tables_vs_fits.json`. world-f9ig |
