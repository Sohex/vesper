...
import { generate, reapplyViaWorker, computeClimateViaWorker, editRecomputeViaWorker, basinAreaFromSlider } from './generate.js';
...
import { buildExportBundle, zipStore } from './data-export.js';
...
for (const [s,v] of [['sN','vN'],['sP','vP'],['sCn','vCn'],['sJ','vJ'],['sNs','vNs'],['sCsv','vCsv'],['sLc','vLc'],['sTw','vTw'],['sS','vS'],['sGl','vGl'],['sHEr','vHEr'],['sTEr','vTEr'],['sRs','vRs'],['sTmp','vTmp'],['sPrc','vPrc'],['sBas','vBas'],['sLit','vLit']]) {
    ...
    slider.addEventListener('input', e => {
        if (s === 'sN') {
            ...
            const km2 = basinAreaFromSlider(+e.target.value);
            document.getElementById(v).textContent =
                km2 === 0 ? 'Off' : km2.toLocaleString() + ' km\u00b2';
        } else {
            document.getElementById(v).textContent = e.target.value;
        }
        if (s === 'sTw' || s === 'sS' || s === 'sGl' || s === 'sHEr' || s === 'sTEr' || s === 'sRs' || s === 'sBas' || s === 'sLit') {
            markReapplyPending();
...
function updatePlanetCode(flash) {
    ...
    const code = encodePlanetCode(
        ...
        +(document.getElementById('sBas')?.value ?? 0),
        +(document.getElementById('sLit')?.value ?? 0),
        getToggledIndices()
    );
    currentCode = code;
...
function paramsToSliderMap(params) {
    return {
        ...
        // Present on codes from this generation; legacy codes decode these to
        // the upstream defaults (basins off, uniform erodibility), which is what
        // those planets actually were.
        sBas: params.basinSlider,
        sLit: params.lithologyStrength,
    };
}
...
function applyCode(code) {
    ...
    for (const [id, val] of Object.entries(map)) {
        if (val === undefined || !document.getElementById(id)) continue;
...
(function initExport() {
    ...
    // Raw data export — every internal field as .bin + manifest.json, zipped.
    const dataGoBtn  = document.getElementById('dataExportGo');
    const scopeEl    = document.getElementById('dataExportScope');
    const dataGridEl = document.getElementById('dataExportGrid');
    const dataDimsEl = document.getElementById('dataExportDims');
    const methodEl   = document.getElementById('dataExportMethod');

    function updateDataDims() {
        const w = +dataGridEl.value;
        dataDimsEl.textContent = w + ' × ' + (w / 2);
    }
    dataGridEl.addEventListener('change', updateDataDims);
    updateDataDims();

    function syncScope() {
        const gridOnly = scopeEl.value !== 'raw';
        dataGridEl.disabled = !gridOnly;
        methodEl.disabled = !gridOnly;
    }
    scopeEl.addEventListener('change', syncScope);
    syncScope();

    dataGoBtn.addEventListener('click', async () => {
        if (!state.curData) return;
        const scope = scopeEl.value;
        const gw = +dataGridEl.value;
        closeModal();
        showBuildOverlay();
        onProgress(0, 'Preparing data export...');
        // Yield so the overlay paints before the synchronous bundling work.
        await new Promise(r => setTimeout(r, 50));
        try {
            const { files, manifest } = buildExportBundle(state.curData, {
                raw: scope !== 'grid',
                grid: scope !== 'raw',
                gridWidth: gw,
                gridHeight: gw / 2,
                gridMethod: methodEl.value,
                onProgress: (frac, label) => onProgress(Math.round(frac * 90), label),
            });
            onProgress(92, 'Packing archive...');
            await new Promise(r => setTimeout(r, 0));
            const zip = zipStore(files);
            const url = URL.createObjectURL(new Blob([zip], { type: 'application/zip' }));
            const link = document.createElement('a');
            link.href = url;
            link.download = `world-orogen-${manifest.seed}.zip`;
            link.click();
            setTimeout(() => URL.revokeObjectURL(url), 10000);
        } catch (e) {
            console.error('[World Orogen] Data export failed:', e);
