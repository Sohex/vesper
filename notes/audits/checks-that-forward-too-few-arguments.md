...
| `aeolian/build_dust_source_fields --self-test` | `build_dust.emission_over_weibull`, survival-space quadrature | a q-space quadrature, and the Fortran patch it mirrors | the two coincide only at zero threshold; the identity is currently unsatisfiable and the self-test is RED and exits 1 | OPEN, `world-2lsg`. Honest failure, and a live divergence between the offline chain and the model rather than a defect in the check |
| `aeolian/build_dust_source_fields` drag | per-cell `z0_aeolian` from the erodible mosaic | a scalar `np.full` in both `build_fields` and the fixture | the self-test's constant `drage` makes the identity pass on that axis whatever the boundary field carries | OPEN, `world-2lsg`, same pass |
