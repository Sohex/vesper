...
| `aeolian/sea_salt_source._earth_check` | "the source functions and the mass integration" | the agreement of two ratios with each other | a common-mode error moves both together; and `_integrate_mass` re-implemented the trapezoid rather than calling `mass_flux` | CLOSED, `world-fgak`. Three bars fixed in advance and both arms routed through `mass_flux`; the absolute one fails and `world-3e2d` carries why |
