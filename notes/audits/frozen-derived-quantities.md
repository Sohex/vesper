...
| the ECOLOGICAL memory, relaxation and resolving brackets | `lib/run_lengths.py` | two, and the strongest form: this pair STATES NO NUMBER. `ecological_timescale_brackets` reads all three out of the acceptance artifacts `assess_lpj_run.py` writes, which record them on a refusal as well as on a pass, so there is nothing declared here to go stale against the measurement. `equilibrium_window_gate.py` checks the derivation as an identity, and checks the ABSENCE of one the same way: an unmeasurable spin-up has to name the estimator that declined it and the count, so a default fails where a derivation would | the biosphere had neither time measured. The RETAINED RECORD floor is now derived from what the acceptance contract must resolve rather than from a span multiple, which did not converge; the SPIN-UP floor has no derivation at all, because the relaxation estimator declines every field-record on disk once a contraction is required to be resolvably below one |
| `nyear_spinup` | `biosphere/scripts/build_vesper_pfts.py` | two, and the loop is live while the derivation is not. It was the shipped LPJ-GUESS 500 carried through a correct YEAR_COUNT rescale, which is Earth's CONVENTION on a right conversion. It now reads the floor from `lib/run_lengths.py` and refuses to rescale it, because a derived value is already in simulation years; the builder falls back to the convention only when the module has no floor to give, and records the module's own reason for that rather than a bare fallback | it stands on the convention today, because the relaxation time it would be derived from is measured on no record this project has. The wire from the measurement to the value is built and tested; what is missing is the measurement, and the emitted file says which of the two it carries |
