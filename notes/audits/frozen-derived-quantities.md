...
| `ph.parent_by_category` (EVERY entry), `calcite_buffer_ph` and its bracket, both `parent_bracket_*`, `endorheic_alkalinity_bonus_bracket` | `pedology/config/pedogenesis.yaml` | one. `pedology/scripts/carbonate_ph.py` solves Slessarev's eq. (6) at this world's `pCO2_bar`; `build_soil.py` fills the sentinels and refuses a literal. `parent_by_category` names the buffer a category reaches at zero export rather than a fresh-parent pH, so the key carries no lithology contrast at all -- that is `ph.base_cation_supply_by_category`, which is declared and sourced and is NOT in this class. The evaporite entry was the last literal: a soda buffer at 8.8 that restated `endorheic_alkalinity_bonus` off the same Helvaci range and on the same land, and `parent_bracket_evaporite` beside it restated `carbonate_ph.py:CLOSED_BASIN_PH_RANGE`. Both are gone | 8.16 to 8.1628, the bar-to-atmosphere conversion. Land-mean soil pH +0.0001 for the conversion; retiring the soda buffer is a further -0.006, and -0.324 with `playa_clastic` leaving the evaporite supply bin beside it |
