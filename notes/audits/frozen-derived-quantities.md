...
| the ECOLOGICAL memory and relaxation brackets | `lib/run_lengths.py` | two, and the strongest form: this pair STATES NO NUMBER. `ecological_timescale_brackets` reads both out of the acceptance artifacts `assess_lpj_run.py` writes, which record them on a refusal as well as on a pass, so there is nothing declared here to go stale against the measurement. `equilibrium_window_gate.py` checks the derivation as an identity rather than reporting it | the biosphere had neither time measured. Memory 2.6-125.3 cycles, relaxation 59.7-846.8 with an open top; the retained-record floor is 1253 cycles and the spin-up floor 2318 |
| `nyear_spinup` | `biosphere/scripts/build_vesper_pfts.py` | two. It was the shipped LPJ-GUESS 500 carried through a correct YEAR_COUNT rescale into 998 simulation years, which is Earth's CONVENTION on a right conversion, and nothing re-derived it when this world's own relaxation time was measured. It now reads the floor from `lib/run_lengths.py` and refuses to rescale it, because the derived value is already in simulation years | 998 to 2318 simulation years. The emitted file carries a DERIVED class of its own so the value cannot be read as a rescale, and the builder falls back to the convention only when no assessed run exists, recording which it used |
