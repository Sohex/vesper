...
| the ECOLOGICAL memory, relaxation and resolving brackets | `lib/run_lengths.py` | two, and the strongest form: this pair STATES NO NUMBER. `ecological_timescale_brackets` reads all three out of the acceptance artifacts `assess_lpj_run.py` writes, which record them on a refusal as well as on a pass, so there is nothing declared here to go stale against the measurement. It pools only artifacts whose recorded contract matches the live one, names the superseded ones and refuses when none is current, so a change to the assessed set cannot leave an old artifact sizing a new run. `equilibrium_window_gate.py` checks each derivation as an identity: the record floor must be the TOP of the resolving bracket exactly, and the spin-up must hold the residual drift inside the tolerance across five decades of relaxation time while one per cent shorter fails at the worst case | the biosphere had neither time measured. The RETAINED RECORD floor is derived from what the acceptance contract must resolve rather than from a span multiple, which did not converge; the SPIN-UP floor is derived from the tolerance ALONE, because the requirement it inverts is bounded over all relaxation times and its supremum satisfies it at every one -- so the relaxation estimator declining every field-record on disk no longer blocks a number |
| `nyear_spinup` | `biosphere/scripts/build_vesper_pfts.py` | two, and both live. It was the shipped LPJ-GUESS 500 carried through a correct YEAR_COUNT rescale, which is Earth's CONVENTION on a right conversion. It now reads the floor from `lib/run_lengths.py` and refuses to rescale it, because a derived value is already in simulation years; the emitted instruction file records which derivation produced it, and the convention is reached only where the live contract has produced no acceptance artifact at all | it is derived today, at the minimax over the unmeasured relaxation time -- 6.86325 times the retained record, a constant fixed by the acceptance tolerance alone. It moves when the tolerance or the record floor moves, and both are read live |
...
| `ph.parent_by_category` (EVERY entry), `calcite_buffer_ph` and its bracket, both `parent_bracket_*`, `soda_buffer_ph` and its bracket | `pedology/config/pedogenesis.yaml` | one. `pedology/scripts/carbonate_ph.py` solves Slessarev's eq. (6) at this world's `pCO2_bar`; `build_soil.py` fills the sentinels and refuses a literal. `parent_by_category` names the buffer a category reaches at zero export rather than a fresh-parent pH, so the key carries no lithology contrast at all -- that is `ph.base_cation_supply_by_category`, which is declared and sourced and is NOT in this class. The evaporite entry was the last literal: a soda buffer at 8.8 that restated a second Helvaci-derived key off the same range and on the same land, and `parent_bracket_evaporite` beside it restated `carbonate_ph.py:CLOSED_BASIN_PH_RANGE`. Both are gone. `soda_buffer_ph` is in this class for a different reason: it does NOT move with pCO2, and what the sentinel buys is that Helvaci's range is written down once, with its citation and its implicit-Earth classification, rather than copied into the config | 8.16 to 8.1628, the bar-to-atmosphere conversion. Land-mean soil pH +0.0001 for the conversion; retiring the soda buffer is a further -0.006, and -0.324 with `playa_clastic` leaving the evaporite supply bin beside it |
