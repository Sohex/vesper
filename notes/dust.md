...
The source area is the dry closed-basin fill. The uncarved fill share, 26.6%
of land, is the hyper-arid limit; carving removes fill (16.5% on
`carved-zoned-v4`, 12.4% on `carved-zoned-v5`) and a flooded playa emits
nothing (4.4 of v4's 16.5 points sit under water), so the dry-source area is
10 to 12% of land -- perhaps two to four times Earth's few-percent
preferential sources, not ten.
...
Reopened before the baseline run, because three consumers now need a
deposition field: loess (SURF-3), the phosphorus return leg above, and the
derived-surface classifier. The first answer was an offline component (now
`aeolian/`, DUST-1) with dust fed forward as a prescribed field, gated by the
threshold below.
...
What the port takes was read out of the source and is designed, measured and
resident per `aeolian/notes/in-model-dust.md`: seven latent upstream defects,
no longwave aerosol term (priced first -- next section), one compile-time
mode, and a bottom-level sink that is a stability hack. The offline chain
stays as the producer of the size-resolved deposition field for pedology and
phosphorus.
...
For the lake, the radiative term dominates: **dust suppresses lake
evaporation, keeps basins overflowing, and a dust-free verdict UNDER-carves**
-- measured below (DUST-10). That is only half the question, because it is
only the lake.
