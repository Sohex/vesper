...
### 11f. Repaired, and verified against the compiled model

The fix extends the numerator rather than shrinking the denominator. An albedo
multiplies the flux over a whole band, so it has to be the reflectance averaged
over that whole band; normalising by the blend grid's own flux would instead
average over a sub-interval and apply the result to all of it. `radmod` now
accumulates `zout1` and `zout2`, the band flux lying outside the blend grid, and
adds each blend's endpoint value across that gap before dividing. All eight
surfaces the block weights are covered, ground and ocean included, because one
`zdenom` pair normalises them all.

Verified by integrating the rebuilt T21 binary for two steps and reading its own
"Finalized Albedos", which matches the predicted values on every one of sixteen
numbers:

| constant | band 1 | band 2 | was, band 1 |
| --- | ---: | ---: | ---: |
| `dsnowalb` | 0.763826 | 0.406115 | 0.752470 |
| `dsnowalbmx` | 0.982784 | 0.585295 | 0.968193 |
| `dsnowalbmn` | 0.507825 | 0.272612 | 0.500267 |
| `dicealbmx` | 0.891810 | 0.472898 | 0.878555 |
| `dicealbmn` | 0.635755 | 0.339491 | 0.626302 |
| `dglacalbmn` | 0.763782 | 0.406194 | 0.752429 |
| `dgroundalb` | 0.193545 | 0.230023 | 0.191674 |
| `doceanalb` | 0.075570 | 0.063668 | 0.074437 |

The two self-consistent repairs differ by at most 0.001, on ground, where
holding an endpoint value across the gap is least representative; the script
reports that residual so the choice stays priced rather than forgotten.

**Every simulated surface is now brighter than it was**, so this makes the
simulation's ice-albedo feedback weaker rather than stronger, and the direction
is the opposite of the one section 11c inferred. The correct term is worth
having whichever way it moves an agreement.

### 11g. A stale binary in a run directory, which rule 4 does not cover

Found while verifying the repair, and recorded because it wasted a rebuild.
`exoplasim/scripts/stability_probe.py` takes its executable from a template
RUN DIRECTORY rather than from the build tree, and a run directory holds a
snapshot. After `rebuild_binaries.py` had rebuilt all five configurations and
`--verify` reported them current, the probe silently ran the template's older
copy and printed the pre-repair albedos.

Rule 4 says a rebuild only refreshes the configuration you ran. This is the
neighbouring failure: the rebuild was complete and correct, and a consumer read
a copy the manifest does not track. The binary manifest records a sha for every
executable it builds, so a consumer that copies one can compare against it.
