...
Fetched and read 2026-08-18 to close MIN-5, grounding the two weakest entries in
`minerals/config/downstream_prospectivity.yaml`. The argument they support is in
`notes/economic-minerals.md`. The short form: the CRITERION is settled for both
elements at once and the RELEASE COEFFICIENT does not exist, which is why both
rules read `grounding: sourced-negative` rather than `declared`.
...
| `helvaci2019-turkish-borate-deposits.pdf` | Helvaci (2019). *Turkish Borate Deposits: Geological Setting, Genesis and Overview of the Deposits.* In Pirajno et al. (eds), Mineral Resources of Turkey, Modern Approaches in Solid Earth Sciences 16, 535-597. `10.1007/978-3-030-02950-0_11` | **read** -- the boron half, and it cuts both ways. It puts the origin at *"andesitic to rhyolitic volcanics"* with the fertile end, quoting Floyd et al. (1998), at ignimbrites that are *"well-evolved and fractionated ... with a high-silica rhyolitic bulk composition, exhibit a combined high content of B, As, F, Li and Pb"*: that combined enrichment is why the borate rule now takes lithium's host set and lost its `arc_basalt` weight. It also lists thermal springs and lake water at pH 8.5-11 among the essential conditions, and reports the Turkish andesite-to-rhyolite suite at 25-270 ppm B attributed to circulating boron-rich hot water. So the ANATOLIAN type genuinely needs the hydrothermal flux this project cannot place, and the Andean type does not |
