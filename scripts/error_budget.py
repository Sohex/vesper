...
FORCING_ITEMS = [
    ...
    ("CH4 and N2O, absent from the longwave", (1.56, 1.98),
     "MEASURED 2026-08-20, CLIM-43, from analysis/trace_gas_forcing.json. "
     "Still the LARGEST item here by a wide margin, against 2.02 W/m2 for "
     "Earth's own CH4 and N2O by the same calculation -- and this world is "
     "BELOW Earth for one reason only, that 1 bar at 12.81 m/s2 is 0.756 of "
     "Earth's column for the same mixing ratio. Byrne and Goldblatt (2014) "
     "cover 100 ppbv to 100 ppmv, which brackets this world where Etminan et "
     "al. (2016) does not, and the two agree to 3.5% where both are valid. "
     "The mixing ratios are THIS STAR'S, and they are its host's grid point "
     "rather than an interpolation: Rugheimer et al. (2013) runs the same "
     "fixed-biogenic-flux photochemistry over 4250 to 7000 K at 250 K spacing "
     "with epsilon Eridani as its 5000 K grid star, which is this host to "
     "within 35 K and is the star config/planet.yaml names its activity on. "
     "That point reads 1.600 ppmv of CH4 and 0.300 ppmv of N2O, the Sun "
     "case's values on both gases. The width was a supposed disagreement with "
     "Segura et al. (2003), and there is none: Segura's K2V is 4620 K, inside "
     "the 4500-4750 K transition where the enhancement turns on, and this "
     "host is above it. What is left is the model's own scatter over the "
     "250 K either side. The range still EXCLUDES the residual below 100 "
     "ppbv, which holds it low, and the CH4-N2O overlap, which the fit cannot "
     "evaluate below its reference N2O and which holds it high. Sasamori's "
     "longwave carries water vapour, CO2 and ozone, so there is no key to "
     "set: adding either gas means adding a band, which is CLIM-42. "
     "THE DESIGN FLUX IS ABSORBING THIS. derive_design_flux.py searches flux "
     "to meet thresholds on the modelled climate, so a missing greenhouse "
     "term is met with more starlight and the temperature comes out right for "
     "the wrong reason. The bias then reaches everything keyed on insolation "
     "rather than temperature, with one sign: photosynthetically active flux, "
     "the shortwave band split, every aerosol forcing that scales with "
     "incident flux, and the reported surface UV."),
