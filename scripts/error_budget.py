...
FORCING_ITEMS = [
    ...
    ("CH4 and N2O, the band's shortfall against line-by-line", (0.76, 1.18),
     ...
     "what belongs here is no longer the whole term but the part of it the "
     "scheme does not apply. Measured: the band lowers global mean outgoing "
     "longwave by 0.799 W/m2 at this world's 1.600 ppmv of CH4 and 0.300 of "
     "N2O against zero, where the offline pricing for the same change is a "
     "little above 2. The range here is that difference. "
     "IT IS A DIFFERENCE OF TWO ESTIMATES, WHICH IS WEAKER THAN THE ITEMS "
     "AROUND IT: the offline half is Byrne and Goldblatt fits checked four "
     "ways, but the model half is one 56-timestep segment on one bed, not a "
     "converged climate. Re-measure it on the first baseline that carries the "
     "band. "
     "THE SIGN IS KNOWN. The band under-delivers, so the modelled world is "
     "short of greenhouse rather than over it, and every threshold that "
     "`derive_design_flux.py` searches against is still met with too much "
     "starlight -- the same attribution error the absent term caused, reduced "
     "by 60 percent rather than removed. "
     "The scheme under-delivers on CO2 by a similar factor, which is the "
     "reason this is booked as a scheme limitation rather than a defect in the "
     "band: its whole CO2 greenhouse is 12.28 W/m2 and its doubling 1.576, "
     "both measured here. The Earth values those invite comparison with are "
     "NOT sourced in this project, so that similarity is an impression and is "
     "not evidence for the range above. "
     "NOT the thinner column: 1 bar at 12.81 m/s2 is 0.756 of Earth's column, "
     "and running the model at the 595 ppm that restores Earth's column moves "
     "the CO2 greenhouse by 0.642 W/m2 -- about four percent of the gap. "
     "The mixing ratios are measured rather than assumed and carry their own "
     "small bracket; CLIM-43 and analysis/trace_gas_forcing.json."),
    ("CH4 and N2O, absent from every run on disk", (1.56, 1.98),
     "The band is resident but NO RUN HAS IT. Zero is the model default and "
     "`config/planet.yaml` only began declaring the mixing ratios on "
     "2026-08-20, so every climatology this project has was integrated without "
     "the term entirely. This row prices what those runs are missing and it "
     "RETIRES the moment a baseline carries the band, at which point the row "
     "above is the whole of it. Keeping both would double-count. "
     "exoplasim/notes/trace-gas-band.md."),
