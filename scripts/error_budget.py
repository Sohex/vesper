...
FORCING_ITEMS = [
    ...
    ("CH4 and N2O, absent from the longwave", (2.25, 5.26),
     "PRICED 2026-08-20, CLIM-41, from exoplasim/notes/trace-gas-absorbers.md. "
     "The LARGEST item here by a wide margin, against 2.02 W/m2 for Earth's own "
     "CH4 and N2O by the same calculation. Byrne and Goldblatt (2014) cover "
     "100 ppbv to 100 ppmv, which brackets this world where Etminan et al. "
     "(2016) does not, and the two agree to 3.5% where both are valid. The "
     "width is NOT the forcing calculation, which is settled to a few percent "
     "by two independent methods: it is a disagreement about whether "
     "tropospheric N2O is enhanced at all, Segura Fig. 12 against Rugheimer's "
     "shielding below 20 km, which is CLIM-43. Both ends still exclude the "
     "residual below 100 ppbv. Sasamori's longwave carries water vapour, "
     "CO2 and ozone, so there is no key to set: adding either gas means adding "
     "a band. The mixing ratios are THIS STAR'S, not Earth's -- Segura et al. "
     "(2003), already the source of star.surface_uv_relative_to_earth and "
     "model.ozone_scale, finds CH4 and N2O systematically higher around a K2V "
     "at the same modern-Earth surface fluxes, because less near-UV means "
     "slower photolysis and longer lifetimes; read off their Fig. 12a at 1 PAL "
     "against Fig. 4 as calibration, a factor of 2 to 5 on both gases. Gravity "
     "cuts the other way and is applied: 1 bar at 12.81 m/s2 makes a mixing "
     "ratio 0.756 of Earth's column. THE DESIGN FLUX IS ABSORBING THIS. "
     "derive_design_flux.py searches flux to meet thresholds on the modelled "
     "climate, so a missing greenhouse term is met with more starlight and the "
     "temperature comes out right for the wrong reason. The bias then reaches "
     "everything keyed on insolation rather than temperature, with one sign: "
     "photosynthetically active flux, the shortwave band split, every aerosol "
     "forcing that scales with incident flux, and the reported surface UV."),
