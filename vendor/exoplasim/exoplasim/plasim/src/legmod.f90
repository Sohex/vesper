...
!     pmat, qmat and gwdc stay threadprivate because they are indexed by the
!     thread's own latitude: the content differs per thread and one shared array
!     would not save a byte.
!
!     THE PER-MODE VECTORS ARE SHARED, and that is the difference between
!     fitting a die and not. They are identical on every thread -- functions of
!     the mode alone -- so a threadprivate copy is the same 0.71 MB eight times
!     over. At T170 that is 35.8 MB a die against CCD1's 32 MB, which misses;
!     shared it is 30.8 MB, which fits. They are written once by legini under a
!     single-thread guard and read-only afterwards, so sharing them needs no
!     synchronisation beyond the barrier that already ends initialisation.
!
!     Fitting is the point of this change rather than a bonus on top of it. On
!     this machine the two-matrix form costs 1.56%; the reason to take that is
!     the parts with more cores and no stacked cache, where a die holds far less
!     than 96 MB and the eight-matrix form has never had a chance.
!$omp threadprivate(pmat,qmat,gwdc,skgpsp,skspgp)
...
! These six are SHARED between threads, so one thread fills them and the rest
! wait. `single` carries that barrier itself and is a comment without -fopenmp,
! so on the MPI and serial builds every process still fills its own copy, which
! is what those builds need. The values are functions of the mode alone and are
! read-only from here on, so nothing after this needs synchronising.
!
! skspgp and skgpsp stay threadprivate and every thread computes them
! identically above, so whichever thread runs this block reads the same filter.
!$omp single
lm = 0
do m = 0 , NTRU
   do n = m , NTRU
