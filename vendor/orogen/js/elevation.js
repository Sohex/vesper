...
import {
    ...
    BASIN_MIN_AREA_KM2, BASIN_MIN_CELLS,
...
import { avgCellAreaKm2 } from './geometry.js';
...
//  land cells).
// ─────────────────────────────────────────────────────────────────────────
function fixupTopology(mesh, r_elevation, r_isOcean, preserveClosedBasins, radiusKm) {
    const { numRegions, adjOffset, adjList } = mesh;
    ...
    // Everything still unvisited and below sea level is a closed below-sea-level
    // depression — a Caspian / Dead Sea / Qattara analogue. Upstream raised all
    // of it to FILL_LEVEL, which destroyed every such basin before erosion even
    // saw the terrain.
    //
    // Now the components are measured first and only sub-resolution ones are
    // filled. The test here is deliberately just "is this a landform the mesh
    // resolves" — the depth-below-spill test needs a spill point, which is not
    // known until the drainage stage, and whether the basin ends up holding
    // water is a question for downstream. Anything kept here that turns out to
    // be noise is filled by the priority flood later anyway.
    if (!preserveClosedBasins) {
        for (let r = 0; r < numRegions; r++) {
            if (!r_isOcean[r] && !visited[r] && r_elevation[r] <= 0) {
                r_elevation[r] = FILL_LEVEL;
            }
        }
        return { filledComponents: 0, keptComponents: 0, keptCells: 0 };
    }

    const cellAreaKm2 = avgCellAreaKm2(numRegions, radiusKm);
    const seen = new Uint8Array(numRegions);
    let filledComponents = 0, keptComponents = 0, keptCells = 0;

    for (let start = 0; start < numRegions; start++) {
        if (seen[start] || r_isOcean[start] || visited[start] || r_elevation[start] > 0) continue;
        const members = [];
        const stack = [start];
        seen[start] = 1;
        while (stack.length > 0) {
            ...
            for (let ni = adjOffset[r], niEnd = adjOffset[r + 1]; ni < niEnd; ni++) {
                const nb = adjList[ni];
                if (seen[nb] || r_isOcean[nb] || visited[nb] || r_elevation[nb] > 0) continue;
                seen[nb] = 1;
                stack.push(nb);
            }
        }
        const resolvable = members.length >= BASIN_MIN_CELLS
                        && members.length * cellAreaKm2 >= BASIN_MIN_AREA_KM2;
        if (resolvable) {
            keptComponents++;
            keptCells += members.length;
        } else {
            filledComponents++;
            for (const r of members) r_elevation[r] = FILL_LEVEL;
        }
    }
    return { filledComponents, keptComponents, keptCells };
}

// ─────────────────────────────────────────────────────────────────────────
//  Main orchestrator
// ─────────────────────────────────────────────────────────────────────────
export function assignElevation(mesh, r_xyz, plateIsOcean, r_plate, plateVec, plateSeeds, noise, noiseMag, seed, spread, plateDensity, superPlateData, r_mantleField, opts = {}) {
    const preserveClosedBasins = opts.preserveClosedBasins !== false;
    const { numRegions } = mesh;
    const _timing = [];
    ...
    // Stage 13 — resolve interior seas (see fixupTopology: sub-resolution
    // closed depressions are filled, resolvable ones are left for the basin
    // machinery downstream)
    const topologyFixup = fixupTopology(mesh, r_elevation, sf.r_isOcean, preserveClosedBasins, opts.radiusKm);
    ...
    return {
        ...
        debugLayers,
        tectonics: buildTectonicsBundle(numRegions, tect, sf, tt),
        topologyFixup,
        _timing
    };
}
...
// ─────────────────────────────────────────────────────────────────────────
//  Tectonic internals bundle
//
//  Stages 1-3 produce a large amount of per-region tectonic state that the
//  renderer never needs (boundary classification, subduction factor, stress
//  orientation, every distance field, archetype weights). It used to die with
//  the stack frame. Everything is collected here so it can be transferred out
//  of the worker and written to disk by the data exporter.
//
//  Sets are converted to Uint8 masks — a Set of ~10^6 region IDs is neither
//  transferable nor practical to serialize.
// ─────────────────────────────────────────────────────────────────────────
function setToMask(numRegions, set) {
    const mask = new Uint8Array(numRegions);
    for (const r of set) mask[r] = 1;
    return mask;
}

function buildTectonicsBundle(numRegions, tect, sf, tt) {
    return {
        // Stage 1 — collision / stress state
        r_stress:          tect.r_stress,
        r_stressDir:       tect.r_stressDir,       // 3 floats per region (unit vector)
        r_subductFactor:   tect.r_subductFactor,
        r_boundaryType:    tect.r_boundaryType,    // 0 interior, 1 convergent, 2 divergent, 3 transform
        r_bothOcean:       tect.r_bothOcean,
        r_hasOcean:        tect.r_hasOcean,
        r_mantleNorm:      tect.r_mantleNorm,      // null when no mantle field was supplied
        mask_mountain:     setToMask(numRegions, tect.mountain_r),
        mask_coastline:    setToMask(numRegions, tect.coastline_r),
        mask_oceanSeed:    setToMask(numRegions, tect.ocean_r),
        mask_stressMountain: setToMask(numRegions, tect.stress_mountain_r),
        maxStress:         tect.maxStress,
        scaleFactor:       tect.scaleFactor,

        // Stage 2 — spatial / distance fields (units: cell hops)
        r_isOcean:         sf.r_isOcean,
        dist_mountain:     sf.dist_mountain,
        dist_ocean:        sf.dist_ocean,
        dist_coastline:    sf.dist_coastline,
        dist_coast:        sf.dist_coast,
        dist_coast_land:   sf.dist_coast_land,
        dist_boundary:     sf.dBdry,
        coastStressMax:    sf.coastStressMax,
        coastSubductMax:   sf.coastSubductMax,
        coastConvergent:   sf.coastConvergent,
        riftDist:          sf.riftDist,
        ridgeDist:         sf.ridgeDist,
        fractureDist:      sf.fractureDist,
        // The three *HalfWidth values are SCALAR band widths for the whole
        // mesh, not per-region fields. They live in bands{} below with the
        // other resolution-dependent widths; keeping them up here made them
        // look like exportable fields, and the exporter silently dropped them.
        riftHalfWidth:     sf.riftHalfWidth,
        ridgeHalfWidth:    sf.ridgeHalfWidth,
        ...
        r_tectonicActivity: tt.r_tectonicActivity,
        r_noiseAmp:        tt.r_noiseAmp,
        r_t_foldBelt:      tt.r_t_foldBelt,
        r_t_craton:        tt.r_t_craton,
        r_t_basin:         tt.r_t_basin,
        r_t_plateau:       tt.r_t_plateau,

        // Resolution-dependent band widths, so a consumer can convert the
        // hop-count distance fields above back into physical distances.
        bands: {
            interiorBand:   sf.interiorBand,
            tectonicReach:  sf.tectonicReach,
            plateauStart:   sf.plateauStart,
            ridgeSigmaBase: sf.ridgeSigmaBase,
            ridgePeakShift: sf.ridgePeakShift,
            ridgeExtent:    sf.ridgeExtent,
            riftHalfWidth:  sf.riftHalfWidth,
            ridgeHalfWidth: sf.ridgeHalfWidth,
            fractureHalfWidth: sf.fractureHalfWidth,
            backArcStart:   sf.baStart,
            backArcPeak:    sf.baPeak,
            backArcEnd:     sf.baEnd,
        },
    };
}
