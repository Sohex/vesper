...
import { basinAreaFromSlider } from './terrain-config.js';
export { basinAreaFromSlider, BASIN_AREA_LADDER_KM2 } from './terrain-config.js';
...
function readLithologyParams() {
    const el = document.getElementById('sLit');
    if (!el) return {};
    const strength = +el.value;
    return { lithology: strength > 0, lithologyStrength: strength };
}

function readBasinParams() {
    const el = document.getElementById('sBas');
    if (!el) return {};
    const areaKm2 = basinAreaFromSlider(+el.value);
    return {
        preserveBasins: areaKm2 > 0,
        basinMinAreaKm2: areaKm2 || undefined,
    };
}
        ...
        switch (msg.type) {
            ...
            case 'done': {
                ...
                state.curData = {
                    ...
                    // Full internal state, kept for the data exporter
                    tectonics: msg.tectonics || null,
                    basins: msg.basins || null,
                    lithology: msg.lithology || null,
                    plates: msg.plates || null,
                    r_plateIndex: msg.r_plateIndex || null,
                    superPlates: msg.superPlates || null,
                    params: msg._params || null,
            ...
            case 'editDone': {
                ...
                if (msg.tectonics) d.tectonics = msg.tectonics;
                if (msg.superPlates) d.superPlates = msg.superPlates;
                if (msg.r_wind_east_summer) {
                    d.r_wind_east_summer = msg.r_wind_east_summer;
                    d.r_wind_north_summer = msg.r_wind_north_summer;
                    d.r_wind_east_winter = msg.r_wind_east_winter;
...
export function reapplyViaWorker(onDone, skipClimate = false) {
    ...
    const s = { ...readSlidersOptional(), ...readBasinParams(), ...readLithologyParams() };
