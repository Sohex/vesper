...
const SLIDERS = [
    ...
    { min: 0,     step: 1,    count: 9   }, // 16: Endorheic Basins (ladder index)
    { min: 0,     step: 0.05, count: 21  }, // 17: Rock Contrast
];

// Mixed-radix bases (right-to-left): litIdx, basIdx, lcIdx, prcIdx, tmpIdx, csvIdx, twIdx,
// scIdx, rsIdx, teIdx, heIdx, glIdx, smIdx, nsIdx, cnIdx, pIdx, jIdx, nIdx, seed
const RADICES = [21, 9, 101, 21, 31, 21, 21, 21, 21, 21, 21, 21, 21, 51, 10, 117, 21, 2556];
const SEED_MAX = 16777216; // 2^24
const BASE_LEN = 24; // base code length (no toggles)
// Previous 22-char codes: everything before the endorheic-basin and rock-contrast
// sliders existed. Those planets genuinely had neither, so they decode to the
// upstream defaults rather than to today's — see the defaults block below.
const PREV6_LEN = 22;
...
const DECODE_FORMATS = {
    ...
    [PREV6_LEN]: {
        ...
        // Codes from before these sliders existed describe planets that were
        // built with upstream drainage and uniform erodibility. Decoding them
        // to today's defaults would silently change the planet, so they get the
        // behaviour they actually had.
        defaults: { basinSlider: 0, lithologyStrength: 0 }
    },
    [BASE_LEN]: {
        radices: RADICES,
        fields: [
            ['lithologyStrength', 17], ['basinSlider', 16],
            ['landCoverage', 15], ['precipitationOffset', 14], ['temperatureOffset', 13],
            ['continentSizeVariety', 12], ['terrainWarp', 11], ['soilCreep', 10], ['ridgeSharpening', 9],
            ['thermalErosion', 8], ['hydraulicErosion', 7], ['glacialErosion', 6],
            ['smoothing', 5], ['roughness', 4], ['numContinents', 3], ['P', 2], ['jitter', 1], ['N', 0],
 ...
 */
export function encodePlanetCode(seed, N, jitter, P, numContinents, roughness, terrainWarp, smoothing, glacialErosion, hydraulicErosion, thermalErosion, ridgeSharpening, soilCreep, continentSizeVariety, temperatureOffset, precipitationOffset, landCoverage, basinSlider = 0, lithologyStrength = 0, toggledIndices = []) {
    const values = {
        N, jitter, P, numContinents, roughness, smoothing, glacialErosion,
        hydraulicErosion, thermalErosion, ridgeSharpening, soilCreep, terrainWarp,
        continentSizeVariety, temperatureOffset, precipitationOffset, landCoverage,
        basinSlider, lithologyStrength,
    };

    // Packed most-significant first, driven by the SAME field table decodeFormat
    // reads. Hand-maintaining a parallel list of RADICES indices here is how the
    // two silently drifted apart before; deriving both from one table makes that
    // failure impossible rather than merely unlikely.
    const { radices, fields } = DECODE_FORMATS[BASE_LEN];
    let packed = BigInt(seed);
    for (let i = fields.length - 1; i >= 0; i--) {
        const [name, si] = fields[i];
        const idx = toIndex(values[name], SLIDERS[si]);
        if (idx < 0 || idx >= SLIDERS[si].count) {
            throw new RangeError(`Planet code: ${name} value ${values[name]} is outside its slider range`);
        }
        packed = packed * BigInt(radices[i]) + BigInt(idx);
    }
