...
import { planChunks } from './draw-chunks.js';
...
// ─────────────────────────────────────────────────────────────────────────
//
// See js/draw-chunks.js for why this exists. Each chunk's attributes are
// SUBARRAY VIEWS onto one backing Float32Array, so they share storage with no
// copy and every colour-update path keeps writing at global side offsets
// exactly as before — only the needsUpdate flag has to be raised per chunk.
// Per-chunk bounding spheres also let the renderer frustum-cull the far side of
// the globe, which one monolithic mesh could never do.

function makeChunkedSurface(pos, col, material) {
    const group = new THREE.Group();
    const chunks = [];
    for (const { start, count } of planChunks(Math.floor(pos.length / 9))) {
        const geo = new THREE.BufferGeometry();
        geo.setAttribute('position',
            new THREE.BufferAttribute(pos.subarray(start * 9, (start + count) * 9), 3));
        const colorAttr = new THREE.BufferAttribute(col.subarray(start * 9, (start + count) * 9), 3);
        geo.setAttribute('color', colorAttr);
        const m = new THREE.Mesh(geo, material);
        group.add(m);
        chunks.push({ mesh: m, colorAttr });
    }
    // NOT userData: Object3D.copy() deep-copies userData through JSON, and a
    // clone of this Group (the map wrap) would hit Meshes and BufferAttributes
    // and throw on the circular structure.
    group._surface = { chunks, colors: col, material };
    return group;
}

/** The full colour array behind a chunked surface, indexed by global offset. */
function surfaceColors(obj) {
    return obj && obj._surface ? obj._surface.colors : null;
}

/** Re-upload every chunk's colour buffer. */
function markColorsDirty(obj) {
    const chunks = obj && obj._surface ? obj._surface.chunks : null;
    if (chunks) for (const c of chunks) c.colorAttr.needsUpdate = true;
}

function disposeSurface(obj) {
    if (!obj) return;
    scene.remove(obj);
    const surf = obj._surface;
    if (!surf) return;
    for (const c of surf.chunks) c.mesh.geometry.dispose();
    if (surf.material) surf.material.dispose();
}

/** Offscreen render helper: one chunked surface in its own Group. */
function addChunkedToScene(offScene, pos, col, material) {
    const g = makeChunkedSurface(pos, col, material);
    offScene.add(g);
    return g;
}
...
export function buildMapMesh() {
    disposeSurface(state.mapMesh); state.mapMesh = null;
    ...
    const base = makeChunkedSurface(new Float32Array(finalPos), new Float32Array(finalCol), mat);

    // Container so the wrap clones inherit visibility and the pan transform.
    // Cloning the Group clones its chunk meshes but shares their geometry and
    // material, so the wrap costs no extra buffers and one needsUpdate on the
    // base chunks covers all three copies.
    const container = new THREE.Group();
    container._surface = base._surface;
    container.add(base);
    const cloneL = base.clone(); cloneL.position.x = -4;
    const cloneR = base.clone(); cloneR.position.x = 4;
    container.add(cloneL, cloneR);

    state.mapMesh = container;
...
export function buildMesh() {
    ...
    disposeSurface(state.planetMesh);
    ...
    // A Group of chunk meshes, not a single Mesh — see makeChunkedSurface.
    // Everything downstream uses .visible / .rotation / .matrixWorld, which a
    // Group provides, and picking is analytical (ray-sphere in edit-mode.js)
    // rather than mesh raycasting, so nothing needs a single geometry.
    state.planetMesh = makeChunkedSurface(pos, col, mat);
...
export function updateMeshColors() {
    ...
    const colors = surfaceColors(state.planetMesh);
    if (!colors) return;
    const { numSides } = mesh;
    const isSmooth = isHeightmap || isLandHeightmap;
    ...
    markColorsDirty(state.planetMesh);
    state._hoverBackup = null;
    state._koppenHoverBackup = null;
    ...
    if (state.mapMesh && state.mapFaceToSide) {
        const mapColors = surfaceColors(state.mapMesh);
        if (!mapColors) return;
...
export function updateMapHoverHighlight() {
    ...
    const colors = surfaceColors(state.mapMesh);
    if (!colors) return;
...
export function updateKoppenHoverHighlight() {
    ...
    if (state.hoveredKoppen >= 0) {
        ...
        if (!koppenArr) { markColorsDirty(state.planetMesh); return; }
        let count = 0;
        for (let s = 0; s < mesh.numSides; s++) {
...
export function updateMapKoppenHoverHighlight() {
    ...
    if (state.hoveredKoppen >= 0) {
        const { mesh, debugLayers } = state.curData;
        const koppenArr = debugLayers && debugLayers.koppen;
        if (!koppenArr) { markColorsDirty(state.mapMesh); return; }
        const fts = state.mapFaceToSide;
        let count = 0;
        for (let f = 0; f < fts.length; f++) {
...
export async function exportMap(type, width, onProgress) {
    ...
    const mapMesh = addChunkedToScene(offScene,
        new Float32Array(posArr.buffer, 0, triCount * 9),
        new Float32Array(colArr.buffer, 0, triCount * 9),
    ...
    // Cleanup mesh
    for (const c of mapMesh._surface.chunks) c.mesh.geometry.dispose();
    mapMesh._surface.material.dispose();
...
export async function exportMapBatch(types, width, onProgress) {
    ...
    for (let ti = 0; ti < total; ti++) {
        ...
        const mapMesh = addChunkedToScene(offScene, posData, colData, mat);
