 ...
 * Ensures every land cell has a monotonically descending drainage path to a
 * drainage terminal, favoring carving through spill points over filling pit
 * floors.
 *
 ...
 *
 * ENDORHEIC BASINS. By default the only terminal is the open ocean, so every
 * land cell is forced to drain to the sea and closed basins are destroyed —
 * Pass 2 exists specifically to cut outlets through them, and on a large basin
 * it will take the better part of a kilometre out of the rim.
 *
 * When `protection` is supplied:
 *   protection.terminalRoot[r] — r is a legitimate terminal. The flood is
 *     seeded from it as well as from the ocean, so cells inside the basin are
 *     reached from the sink outward and drain inward to it. Being reached from
 *     below means their elevation already exceeds the flood surface, so they
 *     are neither filled nor carved.
 *   protection.noLower[r] — r is a basin divide and must never be lowered.
 *     Pass 2 redistributes its carve over the unprotected cells of the kernel
 *     instead, and falls back to filling when the whole window is protected.
 *
 * Ordinary hydraulic, thermal and glacial erosion still run over protected
 * cells. Only the drainage-enforcement carving is held off — the point is to
 * stop the pipeline from deliberately breaching rims, not to freeze terrain.
 */
export function priorityFloodCarve(mesh, r_elevation, r_isOcean, carveStrength, protection) {
    ...
    // Snapshot the protected divides so the invariant below can prove this
    // pass did not lower any of them. The previous attempt at basin
    // preservation failed silently here: routing was changed but the carve
    // still cut the rims, and nothing caught it.
    // Floors the carve may not breach. A fully preserved divide floors at its
    // current elevation; a partially preserved one floors lower, which is how
    // proportional incision is expressed.
    let divideCells = null, divideFloor = null;
    const allow = protection ? (protection.carveAllowance || null) : null;
    if (protection && protection.noLower) {
        divideCells = [];
        for (let r = 0; r < N; r++) if (protection.noLower[r]) divideCells.push(r);
        divideFloor = new Float32Array(divideCells.length);
        for (let i = 0; i < divideCells.length; i++) {
            const r = divideCells[i];
            divideFloor[i] = r_elevation[r] - (allow ? allow[r] : 0);
        }
    }
    ...
    const terminalRoot = protection ? protection.terminalRoot : null;
    const noLower = protection ? protection.noLower : null;
    // Per-cell floor for this pass, indexed by divideCells position; scattered
    // back to a per-region lookup so the carve kernel can clamp by region.
    const minElev = divideCells ? new Float32Array(N) : null;
    if (minElev) {
        minElev.fill(-Infinity);
        for (let i = 0; i < divideCells.length; i++) minElev[divideCells[i]] = divideFloor[i];
    }
    ...
    // Seed: preserved basin sinks. drainTo stays -1 — the sink is where water
    // stops, so it terminates a drain path exactly as the ocean does.
    if (terminalRoot) {
        for (let r = 0; r < N; r++) {
            if (!terminalRoot[r] || r_isOcean[r] || visited[r]) continue;
            visited[r] = 1;
            drainTo[r] = -1;
            heap.push(r);
        }
    ...
    for (let r = 0; r < N; r++) {
        ...
        // Protected divides are excluded from the kernel and their weight is
        // renormalised onto the rest of the window, so the carve still happens
        // — just not through a rim we promised to keep.
        ...
        // Cells floored at their current elevation cannot give anything up, so
        // they are excluded and their share redistributed. Partially protected
        // cells stay in the kernel and are clamped to their floor afterwards.
        // Held iff protected with no allowance. Deliberately NOT "its floor is
        // at its current elevation": pass 3 raises cells, so that test would
        // let a divide the pass had just raised be carved back down.
        const fullyHeld = (c) => !!(noLower && noLower[c] && (!allow || allow[c] <= 0));
        let kernelSum = 0;
        for (let k = startIdx; k <= endIdx; k++) {
            if (fullyHeld(path[k])) continue;
        ...
        if (kernelSum > 0) {
            for (let k = startIdx; k <= endIdx; k++) {
                const c = path[k];
                if (fullyHeld(c)) continue;
                const dist = Math.abs(k - peakIdx);
                const weight = (1 - dist / (radius + 1)) / kernelSum;
                r_elevation[c] -= carveAmount * weight;
                if (minElev && Number.isFinite(minElev[c]) && r_elevation[c] < minElev[c]) {
                    r_elevation[c] = minElev[c];
                }
                if (r_elevation[c] < 0) r_elevation[c] = 0;
            }
        }

        // Fill: raise the pit floor by the remaining fraction. When the whole
        // kernel window was protected nothing could be carved, so the entire
        // deficit becomes fill — the pit ponds instead of the rim being
        // breached, which is the right outcome for a basin we are preserving.
        // Without protection kernelSum is always positive (the peak cell alone
        // contributes weight 1), so this reduces to the original expression.
        const fillAmount = kernelSum > 0 ? deficit * (1 - carveStrength) : deficit;
    ...
    for (let i = 0; i < order.length; i++) {
        ...
        if (target < 0) continue;   // ocean-bound head, or a preserved basin sink
    ...
    if (divideCells) {
        assertDividesNotLowered(divideCells, divideFloor, r_elevation,
            `priorityFloodCarve(carveStrength=${carveStrength})`);
    }
}

/**
 * Invariant: a drainage-enforcement pass may RAISE a protected divide (Pass 3
 * only ever raises) but must never take one below its floor. For a fully
 * preserved basin that floor is the divide's own elevation; for a partially
 * preserved one it sits lower by the allowed incision. A breach here means a preserved
 * basin's rim has been carved and its preservation is a lie — which is exactly
 * how the previous attempt at this failed, silently.
 *
 * Exported so it can be unit-tested directly rather than only through a pass
 * that is designed never to trip it.
 */
export function assertDividesNotLowered(divideCells, before, elevation, context = 'drainage pass') {
    const EPS = 1e-7;
    for (let i = 0; i < divideCells.length; i++) {
        const r = divideCells[i];
        if (elevation[r] < before[i] - EPS) {
            throw new Error(
                `${context} lowered protected divide cell ${r} from ${before[i]} to ` +
                `${elevation[r]} km. Endorheic basin rims must not be carved.`);
        }
    }
}
 ...
 *
 * Thermal: slope-driven material transport, in practice linear hillslope
 * diffusion — see the note on the thermal step below for why the talus
 * threshold does not bind at planetary resolution.
 *
    ...
    glacial)
{
    ...
    // Per-cell stream-power multiplier from the exposed rock. Mean-normalised
    // over land, so lithology redistributes erosion without changing how much
    // of it there is. Null when lithology is off, and the solve then uses the
    // bare K exactly as before.
    const erodibility = litho ? litho.erodibility : null;
    // Snapshot for the exhumation update below: cover is stripped by whatever
    // each cell lost this iteration, and basement erodibility takes over once
    // the cover is gone.
    const elevBefore = litho ? new Float32Array(N) : null;

    // ---- Stream-power normalisation ----
    //
    // flowSeed[r] is the cell's own contributing area expressed as a multiple of
    // the reference cell area, so accumulated flow is a real drainage area and
    // not a cell tally. distScale converts the unit-sphere spacing into the same
    // reference units, so a larger planet gets the larger dx its geometry
    // implies. At Earth and the reference region count the mean seed is 1.000
    // and distScale is exactly 1, preserving the slider calibration — though not
    // bit-identically, since the seed is now each cell's real area rather than a
    // flat 1. Without `hydro` the old cell-count behaviour is retained, which is
    // what the heightmap-import path still uses.
    const refCellKm2 = 4 * Math.PI * HYDRAULIC_REF_RADIUS_KM * HYDRAULIC_REF_RADIUS_KM
                     / HYDRAULIC_REF_REGIONS;
    const flowSeed = hydro && hydro.cellArea ? new Float32Array(N) : null;
    if (flowSeed) for (let r = 0; r < N; r++) flowSeed[r] = hydro.cellArea[r] / refCellKm2;
    const distScale = hydro && hydro.radiusKm ? hydro.radiusKm / HYDRAULIC_REF_RADIUS_KM : 1;
    ...
    for (let iter = 0; iter < totalIters; iter++) {
        ...
        if (elevBefore) elevBefore.set(r_elevation);
        ...
        if (hydraulicThisIter) {
            ...
            for (let i = 0; i < landCount; i++) {
                ...
                // A preserved basin sink is a drainage terminal, exactly like
                // the ocean: water arrives and stops. Leaving drainTarget at -1
                // keeps it out of the stream-power solve below, whose
                // "no cell may sit below its receiver" clamp would otherwise
                // raise the sink to its uphill pit-handling receiver — filling
                // the basin from the floor up, which is how endorheic terrain
                // was being destroyed even with the rims protected.
                if (terminalRoot && terminalRoot[r]) continue;
                ...
                if (bestNb >= 0) {
                    ...
                    cellDist[r] = (neighborDist[bestJ] || 1e-6) * distScale;
                }
            }
            ...
            for (let i = landCount - 1; i >= 0; i--) {
                ...
                const Kr = erodibility ? K * erodibility[r] : K;
                const factor = Kr * Math.pow(flow[r], m) * dt / cellDist[r];
                ...
                //
                // Ocean targets are excluded because sediment reaching the sea
                // leaves the terrestrial elevation model. A preserved basin sink
                // is the same kind of boundary and is excluded for the same
                // reason. Real endorheic basins do aggrade — they are sediment
                // traps — but this rule has no sediment budget and no timescale:
                // every neighbour may raise the sink to its own level, so left
                // enabled it fills the basin from the floor up (~50 m per run on
                // a 190 m basin here) as an artifact rather than as a rate.
                // Downstream can model infill properly against the exported
                // hypsometry, which is the whole point of exporting it.
                const eroded = r_elevation[r] - h_new;
                const targetIsTerminal = r_isOcean[target] || (terminalRoot && terminalRoot[target]);
                if (eroded > 0 && !targetIsTerminal) {
        ...
        // ---- Thermal step ----
        //
        // NOTE ON `talusSlope` — it is not an angle of repose, and cannot be.
        //
        // `slope` below is Δelevation_km / neighbourDistance, where the distance
        // is a chord on the UNIT sphere. The units are mixed, so a real
        // dimensionless gradient is slope / PLANET_RADIUS_KM. The talusSlope the
        // Thermal Erosion slider produces (0.8 at full strength, 1.2 at zero)
        // therefore corresponds to a real angle of 0.007°–0.011°, and measures
        // out as exceeded by 80–86% of downhill land neighbour pairs. It is a
        // near-inert floor, not a repose limit: `totalExcess` ends up being
        // essentially the whole elevation difference, so this step behaves as
        // linear diffusion whose strength is set by kThermal alone.
        //
        // Raising it would not help. A genuine 34° angle of repose needs a
        // model-slope of ~4300; on a 150k-region planet the median neighbour
        // pair sits at 0.037°, the 99th percentile at 0.37°, and the single
        // steepest pair anywhere at 16.9°. Cells are ~50 km apart, so an 8 km
        // cliff spread across one is under 9° — talus slopes are two orders of
        // magnitude below what this grid can represent. Any threshold high
        // enough to mean "angle of repose" would never trigger on any planet.
        //
        // Consequence: the Thermal Erosion slider is a diffusion-strength
        // control. Lithology deliberately does NOT modulate talusSlope, because
        // varying a threshold that 83% of slopes already clear would be tuning
        // noise. Where escarpments would form is reported instead by the
        // scarp_potential field (js/lithology.js), which works with the
        // resolution limit rather than pretending it is not there.
        if (iter < tIters) {
            delta.fill(0);
        ...
        // Progressive exhumation: strip cover by whatever came off this
        // iteration and expose basement where it has run out. This is the point
        // of the two-layer model — a buried craton becomes a stripped shield and
        // an orogen becomes an exposed core, because erosion got there. O(N)
        // arithmetic, negligible against the O(N log N) sort above.
        if (litho && elevBefore) updateExhumation(litho, elevBefore, r_elevation);
    }
